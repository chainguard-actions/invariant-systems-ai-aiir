...
6. **Wait for the three required CI gates** before merge:
...
7. `ai-gate` is an **advisory signal**, not a required merge gate. It is scoped
   to selected sensitive paths and may summarize Copilot review findings on the
   current head commit.
8. **Do not merge without human approval.** In the current solo-maintainer mode,
   no second human approval is required; use judgment and keep merges reviewable.
...
- After fixing, re-check the PR state and any `ai-gate` advisory comment or
   status if the PR touched AI-gated paths.
...
- Internal project names, codenames, or private repository names
- Core IP terms, internal identifiers, or non-public architecture vocabulary
- Non-public hub URLs, service endpoints, or internal hostnames
...
| `tests/` | 2,000+ tests (pytest + Hypothesis) | — |
