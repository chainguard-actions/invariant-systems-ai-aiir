...
python scripts/verify-pypi-provenance.py 1.6.0
...
release provenance coverage. It calls the existing verifier in strict mode and
checks published attestations via PyPI's Integrity API. It is not an offline
verifier; local offline receipt checks still use `aiir --verify`.

Alternatively, query the PyPI Integrity API directly:

```bash
# Fetch attestations for a specific file
curl -s https://pypi.org/integrity/aiir/1.6.0/aiir-1.6.0-py3-none-any.whl/provenance | python3 -m json.tool
```
