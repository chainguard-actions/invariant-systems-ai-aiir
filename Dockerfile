...
# Refresh the glibc runtime while the package index is warm so the release image
# does not keep shipping the vulnerable libc-bin/libc6 pair from the pinned base.
# hadolint ignore=DL3008
RUN apt-get update && \
    apt-get install -y --no-install-recommends git libc-bin libc6 && \
