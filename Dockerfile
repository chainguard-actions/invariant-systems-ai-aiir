...
# Refresh both the glibc runtime and OpenSSL packages while the package index is
# warm so the release image does not keep shipping vulnerable libraries from the
# pinned base image.
...
# Use '*' so that any volume-mounted directory path is trusted regardless of
# the UID mismatch between the host and the aiir user (uid=1000).  The
# alternative (setting safe.directory for only /repo as root) does not work
# because git reads the runtime user's config, not root's.
USER aiir
