...
function serializeViewerCommandArg(value: unknown): unknown {
    if (value instanceof vscode.Uri) {
        return value.toString();
    }

    if (Array.isArray(value)) {
        return value.map(serializeViewerCommandArg);
    }
    ...
    if (value && typeof value === 'object') {
        return Object.fromEntries(
            Object.entries(value as Record<string, unknown>)
                ...
                .map(([key, item]) => [key, serializeViewerCommandArg(item)]),
        );
    }
...
function renderViewerCommandAttributes(command: string, args: unknown[] = []): string {
    return renderCommandAttributes(command, args.map(serializeViewerCommandArg));
}
...
export function getReceiptViewerHtml(
    ...
    const toolContexts = getToolContext(r);
    ...
    const extensionsHtml = (() => {
        ...
        if (toolContexts.length > 0) {
            parts.push('<div class="ext-block"><h3>Companion Tool Context</h3><div class="row">');
            parts.push(`<div class="field"><span class="dim">Summary</span>${escapeHtml(getToolContextSummary(r))}</div>`);
            for (const context of toolContexts) {
                const label = context.tool || context.extension_id || 'unknown tool';
                const details = [context.evidence_level, context.feature, context.tool_version].filter(Boolean).join(' • ');
                const evidence = Array.isArray(context.evidence) && context.evidence.length > 0
                    ? ` • ${context.evidence.join(', ')}`
                    : '';
                parts.push(`<div class="field"><span class="dim">${escapeHtml(label)}</span>${escapeHtml(details || 'context')}${escapeHtml(evidence)}</div>`);
            }
            parts.push('</div></div>');
        }
        ...
        for (const [key, val] of Object.entries(r.extensions)) {
            if (key === 'agent_attestation' || key === 'sigstore' || key === 'editor_provenance' || key === 'tool_context') { continue; }
    ...
    const workspaceUri = record.workspaceFolderPath ? vscode.Uri.file(record.workspaceFolderPath).toString() : undefined;
    const reviewWorkspaceEvidenceCommand = renderViewerCommandAttributes('aiir.showSummary', workspaceUri ? [workspaceUri] : []);
    const generateWithProvenanceCommand = renderViewerCommandAttributes('aiir.generateWithProvenance', workspaceUri ? [workspaceUri] : []);
    const reviewThisCommitCommand = renderViewerCommandAttributes('aiir.reviewReceipt', [{ sha, uri: workspaceUri }]);
    const reviewHistoryCommand = renderViewerCommandAttributes('aiir.viewReviewHistory', workspaceUri ? [workspaceUri] : []);
    const recordExceptionCommand = renderViewerCommandAttributes('aiir.recordException');
    const outcomeActions = [
        !result.valid
            ? repairPlan?.primaryAction
                ? `<button ${renderViewerCommandAttributes(repairPlan.primaryAction.command, repairPlan.primaryAction.args)}>${primaryRepairLabel}</button>`
                : `<button ${renderViewerCommandAttributes('aiir.repairReceipt', [record.uri.toString()])}>Repair Receipt</button>`
            : reviewExpected
                ? `<button ${reviewThisCommitCommand}>Review Commit</button>`
                : '',
        `<button class="secondary" ${renderViewerCommandAttributes('aiir.copyReceiptSummary')}>Copy Markdown Summary</button>`,
        `<button class="secondary" ${renderViewerCommandAttributes('aiir.previewReceiptSummary')}>Preview Markdown</button>`,
        `<button class="secondary" ${renderViewerCommandAttributes('aiir.openReceiptSource')}>Open Receipt JSON</button>`,
    ].filter(Boolean).join('');
    const recoveryActions = !result.valid ? [
        repairPlan?.primaryAction
            ? `<button ${renderViewerCommandAttributes(repairPlan.primaryAction.command, repairPlan.primaryAction.args)}>${primaryRepairLabel}</button>`
            : `<button ${renderViewerCommandAttributes('aiir.repairReceipt', [record.uri.toString()])}>Show Recovery Options</button>`,
        repairPlan?.secondaryActions?.length
            ? `<button class="secondary" ${renderViewerCommandAttributes('aiir.repairReceipt', [record.uri.toString()])}>More Repair Options</button>`
            : '',
        `<button class="secondary" ${renderViewerCommandAttributes('aiir.openReceiptSource')}>${receiptJsonLabel}</button>`,
        `<button class="secondary" ${renderViewerCommandAttributes('aiir.refresh')}>Refresh Receipts</button>`,
    ].filter(Boolean).join('') : '';
...
<body>
    ...
    <section>
        ...
        <div class="callout-panel" style="border-left:3px solid ${evidenceTier === 'signed' ? 'var(--vscode-testing-iconPassed)' : evidenceTier === 'provable' ? 'var(--vscode-charts-blue)' : 'var(--vscode-editorWarning-foreground)'}">
            ...
            ? `<button ${generateWithProvenanceCommand}>Generate With Provenance Next Time</button>`
            : ''}
                ${evidenceTier !== 'signed'
            ? `<button class="secondary" ${renderViewerCommandAttributes('aiir.openSigningGuide')}>Sign This In CI</button>`
            : ''}
                <button class="secondary" ${reviewWorkspaceEvidenceCommand}>Review Workspace Evidence</button>
            </div>
        </div>
    </section>
    ...
    <section>
        ...
        <div class="callout-panel" style="border-left:3px solid ${record.review?.outcome === 'approve' ? 'var(--vscode-testing-iconPassed)' : record.review ? 'var(--vscode-editorWarning-foreground)' : 'var(--vscode-panel-border)'}">
            ...
            <div class="actions">
                <button ${reviewThisCommitCommand}>Review This Commit</button>
                <button class="secondary" ${reviewHistoryCommand}>View Review History</button>
            </div>
        </div>
    </section>
    ...
    <section>
        ...
        <div class="callout-panel" style="border-left:3px solid var(--vscode-editorWarning-foreground)">
            ...
            <div class="actions">
                <button class="secondary" ${recordExceptionCommand}>Record Exception</button>
                <button class="secondary" ${reviewWorkspaceEvidenceCommand}>Review Workspace Evidence</button>
            </div>
        </div>
    </section>
