...
interface CompanionToolContextEntry {
    tool: string;
    extension_id: string;
    feature: string;
...
function detectCompanionToolContext(): CompanionToolContextEntry[] {
    const contexts: CompanionToolContextEntry[] = [];
    const gitlens = vscode.extensions.getExtension('eamodio.gitlens');

    if (gitlens?.isActive) {
        const version = typeof gitlens.packageJSON?.version === 'string'
            ? gitlens.packageJSON.version
            : undefined;
        contexts.push({
            tool: 'GitLens',
            ...
            evidence: ['gitlens_extension_present', 'gitlens_extension_active'],
...
function buildToolContextArgs(): string[] {
    const toolContext = detectCompanionToolContext();
    if (toolContext.length === 0) {
        return [];
    }
    return ['--tool-context', JSON.stringify(toolContext)];
}
...
function getCliPath(): string {
    const configured = getTrustedCliPathSetting();
    if (configured !== 'aiir') {
        return configured;
    }
...
function getConfiguredCliPathSetting(): string {
    return getTrustedCliPathSetting();
}

function getTrustedCliPathSetting(): string {
    const inspected = vscode.workspace.getConfiguration('aiir').inspect<string>('cliPath');
...
function getTrustedHubBaseUrlSetting(): string {
    // Read only from machine/global scope — never from workspaceValue — to
    // prevent a malicious .vscode/settings.json from redirecting the stored
    // hub bearer token to an attacker-controlled server (mirrors the cliPath
    // trust fix in commit 2c28c26).
    const inspected = vscode.workspace.getConfiguration('aiir').inspect<string>('hubBaseUrl');
    const configured = typeof inspected?.globalValue === 'string'
        ? inspected.globalValue
        : typeof inspected?.defaultValue === 'string'
            ? inspected.defaultValue
...
async function updateAiirSetting<T>(key: string, value: T, options?: { ignoreLockPreset?: boolean }): Promise<void> {
    ...
    // cliPath and hubBaseUrl are read ONLY from machine/global scope (a
    // workspace .vscode/settings.json must not redirect the CLI binary or the
    // hub URL the bearer token is sent to). They must therefore be WRITTEN to
    // global scope too — otherwise Connect-to-Hub writes a value the trusted
    // (global-only) reader never sees and the setting can never persist.
    const target = key === 'cliPath' || key === 'hubBaseUrl'
        ? vscode.ConfigurationTarget.Global
        : vscode.ConfigurationTarget.Workspace;

    await vscode.workspace.getConfiguration('aiir').update(key, value, target);
}
...
function buildManagedHook(cliPath: string, args: string[]): string {
    ...
    const toolContextArgs = buildToolContextArgs();
    const trailerArgs = vscode.workspace.getConfiguration('aiir').get<boolean>('injectTrailers', false)
        ? ['--trailer']
        : [];
    const allArgs = [...args, ...buildEditorProvenanceArgs(), ...agentArgs, ...toolContextArgs, ...trailerArgs];
...
class HomeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    // Children for each collapsible section, populated on the top-level
    // getChildren() pass and returned when VS Code expands a section.
    private sectionChildren = new Map<string, vscode.TreeItem[]>();
    ...
    async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
        if (element) {
            // Section parents carry a stable id; return that section's children.
            return (element.id && this.sectionChildren.get(element.id)) || [];
        }
        ...
        // ── Status section children: the health dots. The old flat "headline"
        //    and "Current repository status" rows were non-clickable noise; that
        //    information now lives on the Status section header below.
        ...
        for (const dot of getHomeHealthDots(trustState, targetArgs, latestReceiptArgs)) {
            ...
            statusChildren.push(dotItem);
        }

        // ── Actions section children: the rows that actually DO something,
        //    grouped together so they read as buttons rather than status lines.
        const actionChildren: vscode.TreeItem[] = [];
        ...
        actionChildren.push(actionItem);
        ...
        if (card.secondaryAction) {
            ...
            actionChildren.push(secondaryItem);
        }
        ...
        if (trustState.workspaceOpen) {
            ...
            actionChildren.push(healthItem);
        }

        // ── Hub section children (only when a hub state is resolved).
        const hubChildren: vscode.TreeItem[] = [];
        if (hubState) {
            ...
            hubChildren.push(hubItem);
        }

        // ── Assemble collapsible sections (expanded by default) so status,
        //    actions, and hub are visually separated and clickability is clear.
        this.sectionChildren.clear();
        const sections: vscode.TreeItem[] = [];

        const statusSection = new vscode.TreeItem(card.headline, vscode.TreeItemCollapsibleState.Expanded);
        statusSection.id = 'aiir.section.status';
        statusSection.contextValue = 'aiirSection';
        statusSection.iconPath = new vscode.ThemeIcon(
            trustState.level === 'good' ? 'verified-filled' : trustState.level === 'warn' ? 'warning' : 'info',
        );
        statusSection.description = trustState.repoName
            ? `${trustState.repoName}${trustState.branch ? ` · ${trustState.branch}` : ''}`
            : '';
        // Keep the dot-colour legend on the section that contains the dots.
        statusSection.tooltip = `${card.detail}\n\nGreen means ready, yellow means usable but incomplete, and red means blocked or broken.`;
        this.sectionChildren.set(statusSection.id, statusChildren);
        sections.push(statusSection);

        const actionsSection = new vscode.TreeItem('Actions', vscode.TreeItemCollapsibleState.Expanded);
        actionsSection.id = 'aiir.section.actions';
        actionsSection.contextValue = 'aiirSection';
        actionsSection.iconPath = new vscode.ThemeIcon('zap');
        this.sectionChildren.set(actionsSection.id, actionChildren);
        sections.push(actionsSection);

        if (hubChildren.length > 0) {
            const hubSection = new vscode.TreeItem('Hub', vscode.TreeItemCollapsibleState.Expanded);
            hubSection.id = 'aiir.section.hub';
            hubSection.contextValue = 'aiirSection';
            hubSection.iconPath = new vscode.ThemeIcon('cloud');
            this.sectionChildren.set(hubSection.id, hubChildren);
