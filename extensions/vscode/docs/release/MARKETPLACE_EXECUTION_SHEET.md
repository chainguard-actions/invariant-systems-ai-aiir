...
- Extension version in `package.json`: `0.5.2`
...
3. Keep the VS Code Marketplace token in the canonical local vault and sync it into the GitHub Actions environment `automation-secrets` as `VSCE_PAT`; do not treat repo secrets or shell history as the canonical home.
4. After merge, publish through the next repo release tag or a deliberate sidecar publish run; in both cases the VSIX and Marketplace version come from the extension manifest.
5. Confirm the workflow or local publish logs show all of the following in order: extension version read from manifest, `npm ci`, compile, `npm test`, `npm run marketplace:sync`, VSIX package, VSIX upload or publish.
6. If `VSCE_PAT` is missing, treat the run as incomplete even if the VSIX was attached to GitHub Release.
