...
- Restructured the home sidebar into clear **Status**, **Actions**, and **Hub** sections so the most-used commands are easier to find.

### Security

- Hardened the extension so `aiir.cliPath` is treated as a machine-level setting; workspace-provided values are ignored before invoking the AIIR CLI. Reported by [@EQSTLab](https://github.com/EQSTLab).
- The hub URL (`aiir.hubBaseUrl`) is now stored at global (user) scope and read global-only, matching its trusted-read model so a workspace cannot override it.

## [0.5.2] — 2026-04-30

### Fixed

- Restored panel command dispatch in popped-out webviews so hosted-signup and other browser-facing actions still execute even when inline command rewiring is skipped.

## [0.5.1] — 2026-04-28

### Changed

- Aligned the extension release lane with the AIIR `1.5.1` public-shape refresh so Marketplace/admin release surfaces point at the current coordinated release set.

## [0.5.0] — 2026-04-27

### Added

- Companion GitLens detection during local receipt generation and managed auto-receipting. When `eamodio.gitlens` is active, the extension now records companion context in `extensions.tool_context` rather than treating GitLens as an AI authoring tool.
- Receipt detail surfaces now show a dedicated `Companion Tool Context` section so GitLens context is visible without being conflated with agent attestation.

### Fixed

- Escaped webview command arguments and accepted legacy trailing semicolons in panel actions so Marketplace-installed buttons keep working after CSP hardening.
