...
test('managed auto-receipting carries queued editor provenance into receipt generation', () => {
    expectSource(/const allArgs = \[\.\.\.args, \.\.\.buildEditorProvenanceArgs\(\), \.\.\.agentArgs, \.\.\.trailerArgs\];/, 'expected managed hook generation to include editor provenance consumption');
...
test('webviews enforce CSP, nonce-bearing scripts, bounded commands, and a command allowlist', () => {
    ...
    expectSource(/function wirePanelCommands\(\) \{[\s\S]*querySelectorAll\('\[onclick\]'\)[\s\S]*removeAttribute\('onclick'\);/s, 'expected webview buttons to be rewired away from inline onclick handlers for CSP compatibility');
    expectSource(/document\.addEventListener\('click', event => \{[\s\S]*closest\('\[data-aiir-command\]'\)[\s\S]*cmd\(command, \.\.\.args\);/s, 'expected webview command dispatch to use delegated click handling');
...
test('receipt viewer exposes markdown copy and preview actions', () => {
    ...
    expectSource(/button class="secondary" onclick="cmd\('aiir\.copyReceiptSummary'\)">Copy Markdown Summary<\/button>/, 'expected copy markdown button in receipt viewer');
    expectSource(/button class="secondary" onclick="cmd\('aiir\.previewReceiptSummary'\)">Preview Markdown<\/button>/, 'expected preview markdown button in receipt viewer');
    ...
    expectSource(/renderCommandCall\(repairPlan\.primaryAction\.command, repairPlan\.primaryAction\.args\)/, 'expected invalid receipt viewer primary action to run the repair plan directly');
