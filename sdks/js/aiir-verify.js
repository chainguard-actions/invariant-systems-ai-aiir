/**
 * @invariantsystems/aiir — AIIR receipt verification for JavaScript/TypeScript
 *
...
function _encode(val, depth) {
  ...
  if (t === 'object') {
    // Sort keys by Unicode code point order (not UTF-16 code unit order).
    // Array.from() iterates by code point so [...a] vs [...b] comparisons are
    // code-point-correct even for supplementary-plane characters (> U+FFFF).
    const keys = Object.keys(val).sort((a, b) => {
      const ca = Array.from(a);
      const cb = Array.from(b);
      const len = Math.min(ca.length, cb.length);
      for (let j = 0; j < len; j++) {
        const diff = ca[j].codePointAt(0) - cb[j].codePointAt(0);
