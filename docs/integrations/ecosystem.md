...
It sits before build-provenance, attestation, and transparency-log tools, capturing
authorship context at the moment a change is made.
...
The same model now also applies to non-commit artifacts. AIIR commitment
receipts can bind evaluation plans, manifests, bundle directories, or declared
hashes before those artifacts ever reach CI or a transparency log.
...
| **Agent Trace** | Agent action metadata | Portable attribution and action records across agent tools | AIIR can preserve, hash, sign, or policy-check selected Agent Trace records without requiring Agent Trace to adopt AIIR. See [Agent Trace interoperability](agent-trace-interoperability.md). |
...
Repo-local adapter notes live under `contrib/` for integrations that need a
reviewable implementation or policy mapping:

- [GUAC ingestor adapter](../../contrib/guac/README.md)
- [Witness attestor adapter](../../contrib/witness/README.md)
- [AMPEL policy adapter](../../contrib/ampel/README.md)
...
- [agent-trace-interoperability.md](agent-trace-interoperability.md) — AIIR + Agent Trace interoperability note
- [THREAT_MODEL.md](../../THREAT_MODEL.md) — STRIDE/DREAD analysis
- [verify-independently.md](../reference/verify-independently.md) — verify receipts without trusting AIIR
- [guide-security-team.md](../guides/guide-security-team.md) — integration guide for security teams
