...
in-toto wrapping path. They differ in **what gets receipted**:
...
extensions as informational metadata at this draft stage.
...
Vendors can start at Level 0 or 1 without a hosted service, a PKI rollout, or a
...
The profile is intentionally about action evidence, not prompt wording.

---

## Relationship to the commit-receipt profile

The agent profile is a sibling of the commit-receipt profile. Both profiles share:
...
## Using it (first-party implementation)

As of v1.7.0 AIIR ships a first-party implementation of this profile. The
`aiir agent` subcommand emits and verifies agent receipts; the record_id and
content_hash reproduce the published v0.1 test vectors byte-for-byte.

```bash
# Emit an agent receipt for one action (prints JSON, no ledger write)
aiir agent emit \
  --action edit \
  --tool claude-code \
  --kind agent \
  --summary "Add receipt verification guard" \
  --input file:aiir/cli.py \
  --output file:aiir/cli.py \
  --policy not_evaluated \
  --json

# Or append it to the local ledger (.aiir/receipts.jsonl by default)
aiir agent emit --action test --tool claude-code

# Verify an agent receipt anywhere, offline
aiir agent verify receipt.json          # exit 0 = valid, 1 = tampered/invalid
aiir verify receipt.json                # the generic verifier auto-detects it too
```

Artifacts are `TYPE:REF` pairs (`--input` / `--output`, repeatable); `REF` may
contain colons (e.g. `test:tests/x.py::test_y`). Policy reasons are repeatable
via `--policy-reason`. Receipts are content-addressed (`a1-` + first 32 hex of
the content hash) and carry no file contents, only references.

### Closing the "first adopter sees AI: NO" gap

Chat/agent assistants often leave no durable trailer, so commit receipts come
back `AI: NO`. A Claude Code **PostToolUse** hook emits a declared-agent receipt
for each agent action *before* the commit lands, so the provenance exists even
when the commit message says nothing. See
[claude-code-hooks.md](../guides/claude-code-hooks.md#agent-receipts-posttooluse) for the
hook configuration.
...
1. ~~Validate the draft schema and vectors against a second implementation.~~
   The JS and Rust verifiers round-trip the shared vectors; the Python
   first-party implementation reproduces all three v0.1 agent vectors.
2. ~~Implement a small first-party verifier in AIIR.~~ Shipped as
   `aiir/_agent_receipt.py` (`aiir agent emit|verify`).
...
The canonicalization, hash construction, and vectors are stable at v0.1; the
composition rules and signed-envelope guidance remain draft.
