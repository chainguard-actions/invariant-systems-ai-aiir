...
| `message_hash` | string | yes | `sha256:` plus SHA-256 of the full commit message body, including the subject line (git log --format=%B, whitespace-stripped). |
...
| `tool` | string | yes | Tool-identity URI. MUST be an https URI. Producers SHOULD include a version (e.g. `https://github.com/invariant-systems-ai/aiir@1.6.0`); third-party generators use their own https URI. |
...
5. Verify `schema` is a string beginning with the prefix `'aiir/'`. (Recognized values: `aiir/commit_receipt.v1`, `aiir/commit_receipt.v2`.)
...
CBOR map keys in the canonical envelope MUST be ordered bytewise
(lexicographically) over their encoded CBOR byte sequences per
{{RFC8949}} Section 4.2.1.

The `diff_hash` field is produced by the following canonical git invocation
(see SPEC §3.3 for the full flag list with rationale):

~~~ text
git --no-optional-locks \
  -c core.quotepath=false -c diff.noprefix=false \
  -c diff.algorithm=myers -c diff.context=3 \
  [additional deterministic -c flags] \
...
Independent verifiers MUST use the same flag set to reproduce `diff_hash`.

# Privacy Considerations {#sec-privacy}

AIIR receipts contain personal data in the hashed core fields:

- `commit.author.email` and `commit.committer.email` — email addresses of the
  commit author and committer
- `commit.author.name` and `commit.committer.name` — display names

These fields contribute to `content_hash` and `receipt_id`. Because the receipt
is content-addressed, replacing or removing these fields changes the receipt
identity. This creates tension with the right to erasure under GDPR Art. 17 and
similar data-protection regulations: redacting email from a published receipt
changes its hash and breaks verification of the original.

Producers SHOULD inform contributors that their email addresses are included in
receipts before generating receipts for their commits. For organizations subject
to data-protection regulation, the following mitigations are available:

1. **Per-ledger HMAC pseudonymization** (`--redact-emails`): Replaces email
   addresses with HMAC-SHA256(per-ledger-salt, email) truncated to 16 hex chars,
   formatted as `redacted+<16-hex>@users.noreply.aiir`. This is
   **pseudonymous-per-ledger**, not anonymous: the same email under the same
   salt produces the same pseudonym, enabling deduplication; different ledger
   salts produce different pseudonyms, preventing cross-ledger linkability.
   The local salt is required to reproduce pseudonymized receipt IDs.

2. **Transparency-log permanence**: Receipts submitted to transparency logs
   (Rekor, SCITT ledgers) cannot be deleted. Consider pseudonymization before
   submission rather than after.

3. **Out-of-scope identifiers**: File paths in `files` may also constitute
   personal data in some contexts. The `--redact-files` option omits them.

Consumers SHOULD treat receipts as potentially containing personal data and
