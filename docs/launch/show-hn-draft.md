...
**Trust posture**: 2,499 collected tests, 100% coverage, and 145 public CI checks across
...
GitHub release. OpenSSF Scorecard. We receipt our own repo — every push to
`main` runs a public dogfood workflow that publishes signed receipts to a
separate `receipts` branch.
...
local tamper evidence (hash integrity) to signed non-repudiation.
