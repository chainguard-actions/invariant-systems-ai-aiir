...
```bash
aiir --verify-release --receipts .receipts/ --policy balanced --emit-vsa
```
...
- **Detection of undeclared AI.** AIIR records declared metadata signals. See
  [What AIIR does not do](../integrations/ecosystem.md#what-aiir-does-not-do).
- **Automatic enforcement of contributor trailers.** AIIR records the trailers
  that are present. Enforcing the convention is a social and tooling problem.
