...
## Step 1.5 — Declare AI involvement (important!)

If you used an AI assistant to write code, declare it in the commit message
so AIIR records `AI: YES` instead of `AI: NO`:

```bash
# Commit with a Co-authored-by trailer (AIIR detects this automatically)
git commit -m "feat: add user auth

Co-authored-by: Claude <claude@anthropic.com>"

# Or use an Assisted-by trailer
git commit -m "refactor: clean up API layer

Assisted-by: GitHub Copilot"
```

Then receipt the commit:

```bash
aiir --pretty
```

Without a declaration, AIIR can only detect AI involvement from commit message
keywords and known bot patterns. Chat-based sessions (Copilot Chat, Claude
Code, Cursor Agent) do not leave automatic markers, so you must declare them.

> Without a trailer, the receipt shows `AI: NO`. A year from now, an auditor
> asks "which commits involved AI?" and the receipt answers from the record
...
- **No detection of undeclared AI.** AIIR records what's in commit metadata.
  ...
  See [What AIIR does not do](../integrations/ecosystem.md#what-aiir-does-not-do).
