...
gh attestation verify aiir-1.6.0-py3-none-any.whl \
...
curl -s https://pypi.org/integrity/aiir/1.6.0/aiir-1.6.0-py3-none-any.whl/provenance \
  | python3 -m json.tool
```
...
```bash
aiir --artifact dist/aiir-1.6.0-py3-none-any.whl \
...
100% line coverage is enforced. The current collected-test count is tracked in [standards-readiness.md](standards-readiness.md) and the README proof points.
