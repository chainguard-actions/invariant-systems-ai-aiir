...
> **Module**: `aiir` · **Version**: 1.4.0 · **License**: Apache 2.0
>
...
AIIR can verify **inference receipts** — records that bind model output to
inference parameters via SHA-256 content addressing. This is verify-only:
AIIR checks existing receipts but does not generate them.
...
Verify a single inference receipt's hash integrity. Recomputes
`SHA-256(canonical({granularity, model_fingerprint, sampling_params, tokens}))`
and checks it matches the stored `hash` (or `receipt_hash`) field.

Returns `{"valid": bool, "errors": [...], "receipt_type": "inference_receipt",
"granularity": str, "model_fingerprint": str, "token_count": int}`.

#### `verify_inference_chain`

```python
def verify_inference_chain(receipts: List[Dict[str, Any]]) -> Dict[str, Any]
```

Verify a chain of inference receipts. Checks both individual hash
integrity and `prev_hash` chain linkage between consecutive receipts.

Returns `{"valid": bool, "valid_hashes": int, "valid_links": int,
"total": int, "errors": [...]}`.

#### Auto-detection

`verify_receipt_file()` and `aiir verify` auto-detect inference receipts by
their field signature (`model_fingerprint`, `sampling_params`, `tokens`,
`granularity`). No `--type` flag required — pass any receipt file and AIIR
routes to the correct verifier.

```bash
aiir verify inference_receipt.json
# ✓ Inference receipt verified -- granularity=session, tokens=42, integrity intact.
```
...
Bundle the ledger into a portable JSON export. The format is designed so that downstream governance or analytics systems can ingest it in a single upload. The export now also includes a normalized `evidence_stream` plus an aggregated `evidence_summary` so callers do not need to reverse-engineer tiers, attested tools, or upgrade gaps from raw receipts.
...
Flatten a receipt into one normalized evidence event with fields such as commit identity, attested tool, AI signals, evidence tier, artifact presence, and recommended upgrade path.
...
Convert a portable export bundle, a single receipt, or a list of receipts into a normalized `aiir.evidence_stream.v1` object. When `receipts_dir` is provided, the importer also enriches events with `.cbor` and `.sigstore` sidecar presence from `.receipts/`.
...
Aggregate an evidence stream into governance-oriented outputs, including AI coverage trend, unsigned risk, hot-path AI changes, top attested systems, and release proof readiness.
...
Evaluate ledger-level aggregate policy checks. Returns `(passed, summary, violations)`. Current public aggregate checks include `max_ai_percent` and `max_unsigned_receipts` when configured.
...
| Preset | Use Case | Signing | Max Unsigned | Max AI % |
|--------|----------|---------|--------------|----------|
| `"strict"` | Enterprise / regulated | Required | 0 | 50% |
| `"balanced"` | Standard teams | Recommended | Unlimited | 80% |
| `"permissive"` | Open source / experimentation | Optional | Unlimited | 100% |
