...
## Current Architecture (v1.3.x / Spec v2.0.0)
...
| 7 MCP tools | v1.3.0 | receipt, verify, stats, explain, policy_check, verify_release, gitlab_summary |
| Full Unicode TR39 confusable detection | v1.0.15 | 669 entries across 69 scripts |
| Adversarial red-team hardening | v1.0.15 | 80 hostile tests; threat model now spans 153 security controls |
...
| Release verification engine (`--verify-release`) | v1.2.0 | Policy evaluation + Verification Summary Attestation (VSA) |
| DAG-bound commit receipts | v1.2.2 | `tree_sha` + `parent_shas` bind receipts to exact graph position |
| Review receipts (`--review`) | v1.2.2 | Human review attestations in the same receipt ledger |
| Inference receipt verification | v1.3.0 | `verify_inference_receipt()` and `verify_inference_chain()` |
| Inference-bound evidence tier | v1.3.0 | Trust state between `signed` and `provable` |
...
AIIR's specification is designed for third-party implementation.
Two verifiers currently exist, both maintained by Invariant Systems:

| Language | Package | Scope | Conformant | Maintainer |
| --- | --- | --- | --- | --- |
| Python | [`aiir`](https://pypi.org/project/aiir/) | Full (generate + verify) | ✅ Reference | Invariant Systems |
| TypeScript | [`@aiir/verifier`](contrib/ts-verifier/) | Verification only | ✅ 25/25 core vectors | Invariant Systems |

The TypeScript verifier was written from the specification alone (not ported
from Python) and passes all published test vectors. See
[`contrib/ts-verifier/`](contrib/ts-verifier/) for source and instructions.

**No external-org implementation exists yet.** The conformance manifest
([`schemas/conformance-manifest.json`](schemas/conformance-manifest.json)),
test vectors, and CDDL grammar are published specifically to enable
third-party implementors. If you are interested in building an independent
implementation, start with [docs/implementers.md](docs/implementers.md).
