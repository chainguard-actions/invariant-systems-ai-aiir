...
**CLI version**: 1.7.0
**Date**: 2025-07-15 (updated 2026-03-31)
...
| I-06 | `aiir/_ed25519.py verify()` | The Ed25519 verifier is hand-rolled, pure-Python, and NON-constant-time. Timing measurements of the verification path could in theory leak information about the computation. | Accepted residual risk: verify() processes only PUBLIC keys and signatures — no secret material. Trust roots are operator-provisioned. Timing leakage of public data carries no confidentiality risk. Constant-time guarantees are out of scope for the zero-runtime-dependency, stdlib-only design. Replacement with a constant-time C extension would require adding a runtime dependency. See §9.2. | ⚠️ Accepted risk |
...
**Summary**: No residual **High** or **Critical** risks remain. D-07 (deep JSON)
and I-05 (file enumeration) are mitigated with explicit depth limits and
`--redact-files`.
...
| C7-01 | Crypto | Ed25519 verify() rejects identity/low-order points (anti-forgery); see §3.4/I-06 for non-constant-time disclosure | S |
...
- **Sigstore signing** (`sign: true`): Adds signer authentication,
  transparency-log evidence, and non-repudiation to the tamper-evident receipt.
  Recommended for any compliance-critical deployment.
...
| P1 | Reduce false-negative rate for agent-mode tools (Copilot Chat, Claude Code, Cursor Agent) — see [README § Detection scope](README.md#detection-scope-and-limitations). Today, undeclared agent-mode work can still land in the `human` bucket because these tools do not add durable git markers such as `Co-authored-by` trailers. | S-01 (evasion-by-default) |
...
| P2 | Replace hand-rolled `aiir/_ed25519.py` verifier with a constant-time implementation (e.g., `cryptography` package) if the zero-runtime-dependency constraint is ever relaxed. The current verifier is non-constant-time but processes only public keys/signatures (see I-06). | I-06 |

### 9.3 Operational

- Pin `actions/setup-python` and `actions/upload-artifact` to **full SHA** (already done ✅)
- Pin the CI `cosign` verifier to a patched baseline (`v3.0.5+`) and reject signed bundles whose Rekor body does not self-consistently match the artifact digest, signature, and embedded public key.
...
|---------|------|---------|| 3.2.0 | 2026-03-10 | PEP 740 attestations, PyPI Integrity API verification, SLSA provenance for wheel + sdist, consumer verification script; 5 new supply-chain controls (R22-SC-01–R22-SC-05); transport-level agent attestation (R23-AI-01–R23-AI-04); DAG binding hardening: tree_sha + parent_shas in receipt core (R24-DAG-01–R24-DAG-02); AI_SIGNALS expanded to 48 patterns (was 39); 153 total controls || 3.1.0 | 2026-03-09 | Full Unicode TR39 confusable map — 669 entries across 69 scripts (was 36 hand-curated); S-02 upgraded to Mitigated; 842 tests, 39 AI signals; hostile red-team R9 (80 adversarial tests) |
