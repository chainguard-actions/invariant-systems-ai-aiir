...
run_in_isolated_env() {
  ...
  run_shell "cd '$ROOT_DIR' && '$tmpdir/bin/pip' freeze | grep -iv '^aiir' > _audit_reqs.txt && '$tmpdir/bin/pip-audit' --strict --desc --ignore-vuln CVE-2026-4539 -r _audit_reqs.txt && '$tmpdir/bin/pip-licenses' --format=json --output-file=licenses.json && '$tmpdir/bin/python' scripts/check_licenses.py licenses.json && rm -f _audit_reqs.txt licenses.json"
  rm -rf "$tmpdir"
}
