...
raise SystemExit(0 if marker and marker.exists() else 1)
PY
}

bootstrap_temp_venv_if_needed() {
  if [[ "${AIIR_CI_LOCAL_BOOTSTRAPPED:-0}" == "1" ]]; then
    return
  fi

  if python_in_virtualenv; then
    return
  fi
  ...
  echo
  echo ">>> $PYTHON_BIN is externally managed; bootstrapping a temporary virtualenv for local CI"
  "$PYTHON_BIN" -m venv "$tmpdir"

  local status=0
  (
    export AIIR_CI_LOCAL_BOOTSTRAPPED=1
    export PYTHON="$tmpdir/bin/python"
...
run_in_isolated_env() {
  ...
  run_shell "cd '$ROOT_DIR' && '$tmpdir/bin/pip' freeze | grep -iv '^aiir' > _audit_reqs.txt && '$tmpdir/bin/pip-audit' --strict --desc --ignore-vuln CVE-2026-4539 --ignore-vuln CVE-2026-45409 -r _audit_reqs.txt && '$tmpdir/bin/pip-licenses' --format=json --output-file=licenses.json && '$tmpdir/bin/python' scripts/check_licenses.py licenses.json && rm -f _audit_reqs.txt licenses.json"
  rm -rf "$tmpdir"
}
