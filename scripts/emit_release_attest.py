...
def emit_sidecar_assets(rows: list[dict[str, Any]], asset_dir: Path) -> None:
    asset_dir.mkdir(parents=True, exist_ok=True)
    offline_guide_lines = [
        "# Offline Verification Sidecars",
        "",
        "These files mirror the public release-evidence surface emitted by",
        "`release-attest.json` and `release-attest.md`.",
        "",
        "Each distribution artifact gets:",
        "- `<artifact>.publish.attestation` — AIIR's normalized release-evidence row",
        "- `rekor-<artifact>.json` — extracted Rekor log metadata for that artifact",
        "",
        "Full offline Rekor bundles (`*.rekor-bundle.json`) are published by the",
        "dedicated offline-rekor-release workflow.",
        "",
        "## Included artifacts",
        "",
    ]
    ...
    for row in rows:
        ...
        if not artifact or str(row.get("kind") or "") != "distribution":
            continue

        attestation_payload = {
            "artifact": artifact,
            "kind": str(row.get("kind") or "distribution"),
            "sha256": str(row.get("sha256") or ""),
            "github_release_asset_url": str(row.get("github_release_asset_url") or ""),
            "github_provenance_asset": str(row.get("github_provenance_asset") or ""),
            "github_provenance_asset_url": str(
                row.get("github_provenance_asset_url") or ""
            ),
            "pypi_attestation_url": str(row.get("pypi_attestation_url") or ""),
            "pypi_file_url": str(row.get("pypi_file_url") or ""),
            "rekor_entries": row.get("rekor_entries") or [],
        }
        write_text(
            asset_dir / f"{artifact}.publish.attestation",
            json.dumps(attestation_payload, indent=2) + "\n",
        )
        write_text(
            asset_dir / f"rekor-{artifact}.json",
            json.dumps(row.get("rekor_entries") or [], indent=2) + "\n",
        )
        offline_guide_lines.append(f"- `{artifact}`")

    write_text(asset_dir / "VERIFY-OFFLINE.md", "\n".join(offline_guide_lines) + "\n")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--tag", required=True, help="Release tag, e.g. vX.Y.Z")
    ...
    parser.add_argument(
        "--asset-dir",
        default="",
        help="Optional directory for per-artifact sidecar assets used by release uploads",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.asset_dir:
        emit_sidecar_assets(rows, Path(args.asset_dir))
