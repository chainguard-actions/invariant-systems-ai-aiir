...
class TestCLIGitHubActionOverflowGuard(_CLIBase):
...
class TestSignBoundaries(unittest.TestCase):
    ...
    def test_stat_oserror_on_sign(self):
        """OSError on path.stat() size-check must surface as a ValueError."""
        from aiir._sign import sign_receipt_file
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as f:
            ...
            path = f.name
        try:
            # lstat() succeeds (existence check passes), is_symlink() returns
            # False (not a symlink), then stat() raises — targeting the size-cap
            # branch in sign_receipt_file.
            with (
                patch.object(Path, "lstat", return_value=None),
    ...
    def test_lstat_oserror_on_sign(self):
        """Receipt lstat OSError must surface as a ValueError."""
        from aiir._sign import sign_receipt_file
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as f:
            ...
            path = f.name
        try:
            with patch.object(Path, "lstat", side_effect=OSError("disk error")):
                with self.assertRaises(ValueError) as ctx:
                    sign_receipt_file(path)
                self.assertIn("Cannot stat", str(ctx.exception))
        finally:
...
class TestCLIVerifySignature(_CLIBase):
    ...
    def test_verify_with_signature_pass_prints_bundle_summary(self):
        from aiir.cli import main as cli_main
        from aiir._receipt import write_receipt
        from aiir._receipt import generate_receipt
        ...
        receipt = generate_receipt("HEAD")
        path = write_receipt(receipt, output_dir=self._ledger_dir)
        Path(path + ".sigstore").write_text('{"bundle": true}')

        with (
            patch("aiir.cli.verify_receipt_signature") as mock_verify,
        ...
        self.assertIn(
            "Rekor: hashedrekord, logIndex=321, integrated=2026-04-01T09:03:20Z",
            stderr_text,
        )
    ...
    def test_verify_with_signature_fail_prints_extra_bundle_errors(self):
        from aiir.cli import main as cli_main
        from aiir._receipt import write_receipt
        from aiir._receipt import generate_receipt
        ...
        self.assertEqual(rc, 1)
        stderr_text = err.getvalue()
        self.assertIn("Signature FAILED: bad signature", stderr_text)
