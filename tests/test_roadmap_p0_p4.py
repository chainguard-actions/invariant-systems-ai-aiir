...
from aiir._github import (
    ...
    _format_pr_comment,
    _github_api_request,
    _receipt_assisted_by,
    _receipt_evidence_levels,
    _receipt_has_attested_ai,
    create_check_run,
...
def _make_receipt(**overrides):
    ...
    Computes the real content_hash and receipt_id over the canonical JSON of
    the receipt core so that verify_receipt() returns valid=True.

    Non-core fields (extensions, receipt_id, content_hash, timestamp) are
    handled separately from the hashed core so overriding e.g. extensions
    does not invalidate the computed hash.
    """
    # Non-core fields that must NOT enter the hash computation.
    _NON_CORE_KEYS = {"receipt_id", "content_hash", "timestamp", "extensions"}
    _CORE_KEYS = {"type", "schema", "version", "commit", "ai_attestation", "provenance"}

    # Split overrides into core vs non-core.
    core_overrides = {k: v for k, v in overrides.items() if k not in _NON_CORE_KEYS}
    non_core_overrides = {k: v for k, v in overrides.items() if k in _NON_CORE_KEYS}

    # Base core — these are the fields hashed by verify_receipt.
    core = {
    ...
    # Apply core-level overrides before computing the hash.
    core.update(core_overrides)

    # Compute real content-addressed fields so verify_receipt returns valid=True.
    receipt_core = {k: v for k, v in core.items() if k in _CORE_KEYS}
    ...
    # Apply non-core overrides last (they overwrite the defaults above).
    base.update(non_core_overrides)
    return base
...
def _make_ai_receipt(**overrides):
    ...
    Passes the ai_attestation changes through _make_receipt so the
    content_hash and receipt_id are computed over the correct core values.
    """
    ai_attestation = {
        "is_ai_authored": True,
    ...
    return _make_receipt(ai_attestation=ai_attestation, **overrides)
...
class TestFormatCommitTrailer(unittest.TestCase):
    ...
    def test_declared_agent_trailer_marks_ai_and_tool(self):
        """Declared agent attestation must flip AIIR-AI and emit tool/evidence trailers."""
        receipt = _make_receipt(
            extensions={
                "agent_attestation": {
                    "tool_id": "gitlens",
                    "confidence": "declared",
                    "run_context": "ide:active",
                }
            }
        )
        trailer = format_commit_trailer([receipt])
        self.assertIn("AIIR-AI: true", trailer)
        self.assertIn("AI-Assisted-By: gitlens", trailer)
        self.assertIn("AIIR-Evidence-Level: declared", trailer)

    def test_companion_tool_context_only_stays_contextual(self):
        """Companion tool_context must not be promoted to AI authorship by itself."""
        receipt = _make_receipt(
            extensions={
                "tool_context": [
                    {
                        ...
                        "evidence": ["gitlens_extension_present"],
                    }
                ]
            }
        )
        trailer = format_commit_trailer([receipt])
        self.assertIn("AIIR-AI: false", trailer)
        self.assertNotIn("AI-Assisted-By:", trailer)
        self.assertNotIn("AIIR-Evidence-Level:", trailer)

    def test_gitlens_commit_composer_context_emits_discoverable_trailers(self):
        """Explicit GitLens Commit Composer context should be visible in trailers."""
        receipt = _make_receipt(
            extensions={
                "tool_context": [
                    {
        ...
        trailer = format_commit_trailer([receipt])
        self.assertIn("AIIR-AI: true", trailer)
        self.assertIn("AI-Assisted-By: GitLens Commit Composer", trailer)
        self.assertIn("AIIR-Evidence-Level: declared+ide-context", trailer)

    def test_gitlens_commit_composer_assisted_by_label_deduplicates(self):
        """Agent and companion labels should not duplicate the same GitLens tool."""
        receipt = _make_receipt(
            extensions={
                "agent_attestation": {
                    "tool_id": "GitLens Commit Composer",
                    "confidence": "declared",
                    "run_context": "ide:active",
                },
                "tool_context": [
        ...
        trailer = format_commit_trailer([receipt])
        self.assertEqual(trailer.count("AI-Assisted-By:"), 1)
        self.assertIn("AI-Assisted-By: GitLens Commit Composer", trailer)

    def test_declared_and_provenance_evidence_levels_compose(self):
        """Declared agent context plus editor provenance should emit combined evidence levels."""
        receipt = _make_receipt(
            extensions={
                "agent_attestation": {
                    "tool_id": "copilot",
        ...
        trailer = format_commit_trailer([receipt])
        self.assertIn(
            "AIIR-Evidence-Level: declared+ide-context+deterministic-provenance",
            trailer,
        )

    def test_assisted_by_ignores_non_dict_agent_attestation(self):
        """Malformed agent attestation must not produce an assisted-by trailer value."""
        receipt = _make_receipt(extensions={"agent_attestation": "gitlens"})
        self.assertIsNone(_receipt_assisted_by(receipt))

    def test_editor_provenance_alone_counts_as_attested_ai_activity(self):
        """Deterministic editor provenance should mark a receipt as AI-involved even without agent_attestation."""
        receipt = _make_receipt(
            extensions={"editor_provenance": {"records": [{"id": "rec-1"}]}}
        )
        self.assertTrue(_receipt_has_attested_ai(receipt))
        trailer = format_commit_trailer([receipt])
        self.assertIn("AIIR-AI: true", trailer)

    def test_evidence_levels_cover_trailer_signal_transport_and_malformed_context(self):
        """Evidence-level helper should accept trailer declarations and ignore malformed tool-context entries."""
        receipt = _make_receipt(
            ai_attestation={
                **_make_receipt()["ai_attestation"],
                "signals_detected": ["trailer:copilot"],
            },
            extensions={
                "agent_attestation": {
                    "run_context": "mcp",
                ...
                "editor_provenance": {"records": []},
            },
        )

        self.assertEqual(
            _receipt_evidence_levels(receipt),
            ["declared", "ide-context", "mcp-observed"],
        )

    def test_evidence_levels_do_not_duplicate_existing_mcp_or_tool_context_levels(self):
        """Duplicate evidence levels across receipts should collapse to a single trailer entry."""
        receipt = _make_receipt(
            extensions={
                "agent_attestation": {
                    "tool_id": "copilot",
                ...
                "tool_context": [
                    {"evidence_level": "mcp-observed"},
                    {"evidence_level": "ide-context"},
                ],
            }
        )
        trailer = format_commit_trailer([receipt, receipt])
        self.assertIn("AIIR-Evidence-Level: declared+mcp-observed+ide-context", trailer)
        self.assertEqual(trailer.count("mcp-observed"), 1)
...
class TestInitPolicyEdgeCases(unittest.TestCase):
    ...
    def test_init_policy_unknown_preset_is_rejected(self):
        """--init --policy unknown-preset now errors instead of silently falling back.

        C5 security hardening: unknown policy presets are rejected with a
        nonzero exit code and an error message listing valid presets.
        This test MUST fail if someone reverts to the old silent-fallback behavior.
        """
        import io
        import contextlib
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                os.chdir(old_cwd)
            # Unknown preset must be rejected with a nonzero exit code.
            self.assertEqual(result, 1)
            # Error message must list valid presets so the user knows what to use.
            stderr = stderr_buf.getvalue()
            self.assertIn("unknown-preset", stderr)
            self.assertIn("balanced", stderr)
            self.assertIn("strict", stderr)
            self.assertIn("permissive", stderr)
            # policy.json must NOT have been created — the init was aborted.
...
class TestInitPolicyValueError(unittest.TestCase):
    ...
    def test_policy_init_value_error_fails_init(self):
        """If init_policy raises ValueError, --init now exits with code 1.

        C5 security hardening: a ValueError from init_policy (e.g. unknown or
        malformed preset) is a hard error, not a silent no-op.  The command
        must exit non-zero so CI pipelines catch misconfiguration.
        This test MUST fail if someone reverts to the old ignore-and-succeed behavior.
        """
        import io
        import contextlib
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                os.chdir(old_cwd)
            # ValueError from init_policy must produce a nonzero exit code.
            self.assertEqual(result, 1)
            # The error message must be surfaced to stderr.
            self.assertIn("bad policy", stderr_buf.getvalue())
            # policy.json must NOT exist (init_policy raised before writing it).
            self.assertFalse((aiir_dir / "policy.json").exists())
            # Other ledger files may or may not exist depending on ordering;
            # the important invariant is that the CLI does NOT silently succeed.
