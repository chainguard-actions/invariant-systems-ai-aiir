...
class TestReceiptIntegrity(unittest.TestCase):
    ...
    def test_redact_emails_replaces_git_identity_emails(self):
        """Email redaction must use HMAC-SHA256(fallback-salt, email)[:16].

        Three invariants are checked:
          1. The token matches the HMAC-SHA256 value derived with the fixed
             fallback salt (b'aiir.redaction.v2'), proving the old unsalted
             sha256 is gone and the new HMAC path is active.
          2. Calling build_commit_receipt twice with no explicit salt is
             deterministic — same salt+email always produces the same pseudonym.
          3. Two different salts produce different pseudonyms for the same email
             (cross-ledger non-linkability).  A revert of the HMAC fix would
             break this because sha256(email) is globally constant.
        """
        import hashlib
        import hmac as _hmac

        # Expected token: HMAC-SHA256(b'aiir.redaction.v2', 'test@example.com')[:16]
        _FALLBACK_SALT = b"aiir.redaction.v2"
        normalized = "test@example.com"
        expected_digest = _hmac.new(
            _FALLBACK_SALT, normalized.encode("utf-8"), hashlib.sha256
        ).hexdigest()[:16]
        expected_token = f"redacted+{expected_digest}@users.noreply.aiir"

        # 1. Token matches HMAC-based derivation (not the old sha256 value).
        commit = self._make_dummy_commit()
        receipt = cli.build_commit_receipt(commit, redact_emails=True)
        author_email = receipt["commit"]["author"]["email"]
        committer_email = receipt["commit"]["committer"]["email"]
        self.assertEqual(author_email, expected_token)
        self.assertEqual(author_email, committer_email)
        self.assertNotIn("test@example.com", json.dumps(receipt))
        # Ensure the old unsalted sha256 value is no longer produced.
        self.assertNotEqual(
            author_email, "redacted+973dfe463ec85785@users.noreply.aiir"
        )

        # 2. Determinism: same implicit salt+email always yields the same pseudonym.
        receipt2 = cli.build_commit_receipt(commit, redact_emails=True)
        self.assertEqual(
            receipt2["commit"]["author"]["email"],
            author_email,
            "Same salt+email must produce the same pseudonym (determinism).",
        )

        # 3. Cross-ledger non-linkability: different explicit salts produce
        #    different pseudonyms, so user identities cannot be linked across ledgers.
        salt_a = "a" * 64  # valid 256-bit hex salt
        salt_b = "b" * 64  # different valid 256-bit hex salt
        receipt_a = cli.build_commit_receipt(
            commit, redact_emails=True, redaction_salt=salt_a
        )
        receipt_b = cli.build_commit_receipt(
            commit, redact_emails=True, redaction_salt=salt_b
        )
        token_a = receipt_a["commit"]["author"]["email"]
        token_b = receipt_b["commit"]["author"]["email"]
        self.assertNotEqual(
            token_a,
            token_b,
            "Different salts must produce different pseudonyms (cross-ledger non-linkability).",
        )

    def test_redact_emails_uses_fallback_for_blank_identity(self):
        commit = self._make_dummy_commit()
        commit.author_email = "   "
        commit.committer_email = "\x1b[31m\x1b[0m"

        receipt = cli.build_commit_receipt(commit, redact_emails=True)

        self.assertEqual(
            receipt["commit"]["author"]["email"],
            "redacted@users.noreply.aiir",
        )
        self.assertEqual(
            receipt["commit"]["committer"]["email"],
            "redacted@users.noreply.aiir",
        )
...
class TestPublicBasic(unittest.TestCase):
    ...
    def test_help_text_contains_url(self):
        ...
        self.assertIn("invariantsystems.io", stdout.getvalue())
