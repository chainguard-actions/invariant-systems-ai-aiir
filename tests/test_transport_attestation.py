...
class TestToolContextExtension(unittest.TestCase):
    """Companion tool_context metadata should flow into commit receipts cleanly."""

    def test_mcp_manifest_defines_gitlens_commit_composer_workflow(self):
        manifest_path = Path(__file__).resolve().parent.parent / "mcp-manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        workflows = {item["name"]: item for item in manifest.get("workflows", [])}

        workflow = workflows["gitlens_commit_composer_receipt"]
        self.assertEqual(workflow["tool"], "aiir_receipt")
        self.assertEqual(workflow["toolContext"]["feature"], "commit_composer")
        self.assertIn("AI-Assisted-By: GitLens Commit Composer", workflow["trailers"])

    def test_sanitize_tool_context_none_returns_empty(self):
        from aiir._receipt import _sanitize_tool_context

        self.assertEqual(_sanitize_tool_context(None), [])

    def test_sanitize_tool_context_filters_invalid_entries(self):
        from aiir._receipt import _sanitize_tool_context

        entries = _sanitize_tool_context(
            [
                "not-a-dict",
                {
                    "tool": "GitLens",
                ...
                {
                    "tool": "",
                    "evidence": ["", "gitlens_extension_active"],
                },
                {"evidence": []},
            ]
        )
    ...
    @patch("aiir._receipt._run_git", return_value="https://example.com/repo.git")
    def test_build_commit_receipt_includes_sanitized_tool_context(self, _mock_git):
        from aiir._detect import CommitInfo
        ...
        entries = receipt["extensions"]["tool_context"]
        self.assertEqual(len(entries), 1)
        self.assertEqual(entries[0]["tool"], "GitLens")
        self.assertEqual(entries[0]["extension_id"], "eamodio.gitlens")
        self.assertNotIn("unexpected", entries[0])

    @patch("aiir.cli.generate_receipt")
    @patch("aiir.cli.get_repo_root", return_value="/tmp/fake-repo")
    def test_cli_forwards_tool_context_json(self, _root, mock_gen):
        mock_gen.return_value = {
            "type": "aiir.commit_receipt",
            "receipt_id": "g1-test",
            "content_hash": "sha256:test",
        ...
        cli.main(
            [
                "--json",
                "--tool-context",
                '[{"tool":"GitLens","evidence_level":"ide-context"}]',
                "--commit",
                "HEAD",
            ]
        )
    ...
    @patch("aiir.cli.get_repo_root", return_value="/tmp/fake-repo")
    def test_cli_rejects_invalid_tool_context_json(self, _root):
        import aiir.cli as cli

        stderr = io.StringIO()
        with patch("sys.stderr", stderr):
            rc = cli.main(["--json", "--tool-context", "{bad", "--commit", "HEAD"])

        self.assertEqual(rc, 1)
        self.assertIn("Invalid --tool-context JSON", stderr.getvalue())

    @patch("aiir.cli.get_repo_root", return_value="/tmp/fake-repo")
    def test_cli_rejects_scalar_tool_context_json(self, _root):
        import aiir.cli as cli

        stderr = io.StringIO()
        with patch("sys.stderr", stderr):
            rc = cli.main(["--json", "--tool-context", "123", "--commit", "HEAD"])

        self.assertEqual(rc, 1)
        self.assertIn("must decode to a JSON object or array", stderr.getvalue())
