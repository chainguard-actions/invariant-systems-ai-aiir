...
def _make_receipt(**overrides: Any) -> Dict[str, Any]:
    ...
    base: Dict[str, Any] = {
        ...
        "commit": {
            "sha": "abc123def456789",
            "tree_sha": "def456abc123789",
            "parent_shas": ["000111222333444"],
...
class TestInTotoWrapper(unittest.TestCase):
    ...
    def test_subject_without_remote(self):
        ...
        receipt = _make_receipt(provenance={"repository": None, "tool": "aiir@1.3.0"})
        stmt = wrap_in_toto_statement(receipt)
    ...
    def test_predicate_type_is_stable_uri(self):
        """Predicate type URI is the documented stable current value."""
        self.assertEqual(
            INTOTO_PREDICATE_TYPE,
            "https://invariantsystems.io/predicates/aiir/commit_receipt/v2",
        )

    def test_legacy_v1_receipt_uses_v1_predicate_type(self):
        """Legacy v1 receipts retain the matching v1 predicate URI."""
        receipt = _make_receipt(schema="aiir/commit_receipt.v1")
        stmt = wrap_in_toto_statement(receipt)
...
class TestInTotoIntegration(unittest.TestCase):
    ...
    def test_wrapped_receipt_retains_all_fields(self):
        ...
        self.assertEqual(predicate["schema"], "aiir/commit_receipt.v2")
