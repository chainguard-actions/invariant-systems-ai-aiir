...
class TestRedTeamHardeningR4(unittest.TestCase):
    ...
    def test_action_yml_sigstore_version_has_ceiling(self):
        """R4-09: action.yml sigstore install should use hash-pinned requirements."""
        ...
        if action_path.exists():
            ...
            self.assertIn(
                "--require-hashes",
                content,
                "action.yml should install sigstore with --require-hashes",
            )
            self.assertIn(
                ".github/requirements/sigstore.txt",
                content,
                "action.yml should reference the pinned sigstore requirements file",
            )
...
class TestPublicBasicR8(unittest.TestCase):
    ...
    def test_r8_pub_01_security_md_references_current_major_minor(self):
        ...
        expected_series = ".".join(cli.CLI_VERSION.split(".")[:2]) + ".x"
        self.assertIn(expected_series, content)
