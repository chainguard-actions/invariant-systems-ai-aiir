...
# Website-coupled checks only make sense when the sibling site checkout is both
# present and still on the matching AIIR contract revision.
SITE_CHECKOUT_EXISTS = SITE_DIR.is_dir() and (SITE_DIR / "index.html").is_file()
...
def _site_checkout_matches_contract() -> bool:
    if not SITE_CHECKOUT_EXISTS:
        return False

    try:
        from aiir import __version__

        stats_version = json.loads(_read(SITE_DIR / "stats.json"))["version"]
        if stats_version != __version__:
            return False

        integrations = _read(SITE_DIR / "integrations.html")
        if re.search(
            r'<meta[^>]+http-equiv="refresh"[^>]+/docs#integrations',
            integrations,
        ):
            return False
    except Exception:
...
SITE_EXISTS = _site_checkout_matches_contract()
...
class TestWebsiteHTMLIntegrity(unittest.TestCase):
    ...
    def test_nav_consistency_12_links(self):
        """Every main page header <nav> has the current 12-link contract."""
        expected_hrefs = {
            "/",
            "/docs",
            "/verify",
        ...
        for page in _main_pages():
            ...
            self.assertEqual(
                len(hrefs),
                12,
                f"{page.name}: expected 12 nav links, got {len(hrefs)}: {hrefs}",
            )
    ...
    def test_footer_consistency(self):
        ...
        for page in _main_pages():
            ...
            expected_count = expected_counts.get(page.name, 15)
            self.assertEqual(
                len(hrefs),
                expected_count,
            ...
            self.assertIn(
                ...
                hrefs,
                f"{page.name}: footer missing GitLab link",
            )
    ...
    def test_canonical_urls_on_all_pages(self):
        ...
        for page in _main_pages():
            ...
            url = match.group(1)
            host_ok = url == "https://invariantsystems.io" or url.startswith(
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def _docs_api_version(self) -> str:
        content = _read(AIIR_ROOT / "docs" / "reference" / "api.md")
        match = re.search(r"\*?\*?[Vv]ersion\*?\*?[:\s]+([\d]+\.[\d]+\.[\d]+)", content)
        return match.group(1) if match else "NOT_FOUND"

    def test_all_versions_match(self):
        """__init__ == manifest == stats.json == .well-known/mcp.json == docs/reference/api.md"""
        ...
        versions = {
            ...
            "docs/reference/api.md": self._docs_api_version(),
        }
...
class TestAIDetectionAccuracy(unittest.TestCase):
    """The integrations page declared-signal table matches actual detection code."""

    def _page_detected_tools(self) -> Set[str]:
        """Tool names from the declared-signal table on the integrations page."""
        content = _read(SITE_DIR / "integrations.html")
        # Find the declared-signal table (renamed from "AI Tools Detected")
        section = re.search(
            r"Declared Signals and Tool Context.*?<tbody>(.*?)</tbody>",
            content,
            re.DOTALL,
        )
        self.assertIsNotNone(section, "Declared signals table not found")
        # Extract first <td> from each row (the tool name)
        return set(re.findall(r"<tr>\s*<td>(.*?)</td>", section.group(1)))
...
class TestREADMECLIContract(unittest.TestCase):
    ...
    def test_readme_dogfood_claim_points_to_live_public_surface(self):
        """Dogfood proof row must point at the live receipts branch + workflow."""
        readme = _read(AIIR_ROOT / "README.md")
        self.assertIn(
            "https://github.com/invariant-systems-ai/aiir/tree/receipts",
            readme,
        )
        self.assertIn(
            "https://github.com/invariant-systems-ai/aiir/actions/workflows/dogfood.yml",
            readme,
        )

    def test_dogfood_case_study_distinguishes_live_feed_from_snapshot(self):
        """Dogfood case study must distinguish receipts branch from .aiir snapshot."""
        case_study = _read(AIIR_ROOT / "docs" / "case-studies" / "aiir-self-dogfood.md")
        self.assertIn("receipts` branch", case_study)
        self.assertIn("checked-in `.aiir/` directory on `main`", case_study)
        self.assertIn("aiir --verify .aiir/receipts.jsonl", case_study)

    def test_aiir_snapshot_readme_explains_scope(self):
        """Tracked .aiir snapshot must explain it is not the live CI feed."""
        snapshot_readme = _read(AIIR_ROOT / ".aiir" / "README.md")
        self.assertIn("checked-in local ledger snapshot", snapshot_readme)
        self.assertIn("live CI dogfood feed", snapshot_readme)
        self.assertIn("receipts` branch", snapshot_readme)

    def test_unreleased_changelog_dogfood_entry_matches_public_surface(self):
        """Unreleased changelog text must keep snapshot and live dogfood paths separate."""
        changelog = _read(AIIR_ROOT / "CHANGELOG.md")
        unreleased, _, _ = changelog.partition("## [1.5.1]")
        self.assertIn(".aiir/receipts.jsonl", unreleased)
        self.assertIn("public `receipts` branch", unreleased)
        self.assertNotIn("under `.aiir/receipts/`", unreleased)

    def test_public_repo_does_not_ship_launch_commentary_files(self):
        for rel_path in [
            AIIR_ROOT / "docs" / "launch" / "show-hn-draft.md",
            AIIR_ROOT / "docs" / "launch" / "show-hn-comment.txt",
        ]:
            self.assertFalse(
                rel_path.exists(), f"{rel_path.name} should not ship publicly"
            )

    def test_public_dogfood_docs_do_not_reintroduce_stale_count_claims(self):
        """Public dogfood surfaces must not hardcode stale snapshot counts as proof."""
        stale_phrases = [
            "199 of 252 dogfood receipts",
            "of 252 dogfood receipts, 199",
            "public ledger",
        ]
        public_surfaces = [
            AIIR_ROOT / "README.md",
            AIIR_ROOT / "docs" / "case-studies" / "aiir-self-dogfood.md",
        ]
...
class TestSchemaIntegrity(unittest.TestCase):
    ...
    def test_commitment_receipt_schema_valid_json(self):
        path = AIIR_ROOT / "schemas" / "commitment_receipt.v1.schema.json"
        self.assertTrue(path.is_file(), "commitment receipt schema missing")
        schema = json.loads(_read(path))
        self.assertIn("$schema", schema)
        self.assertIn("properties", schema)
...
class TestIntegrationsPageClaims(unittest.TestCase):
    ...
    def test_6_ai_assistant_cards(self):
        """The AI assistants section has exactly 6 assistant cards."""
        content = _read(SITE_DIR / "integrations.html")
        # Accept both old ("AI Coding Assistants") and new ("AI Assistants via MCP") headings
        section = re.search(
            r"AI (?:Coding )?Assistants(?:\s+via MCP)?.*?MCP (?:Server )?Tools",
            content,
            re.DOTALL,
        )
        self.assertIsNotNone(section)
    ...
    def test_7_mcp_tools_in_table(self):
        ...
        content = _read(SITE_DIR / "integrations.html")
        section = re.search(
            r"MCP (?:Server )?Tools.*?Declared Signals and Tool Context",
            content,
            re.DOTALL,
        )
        self.assertIsNotNone(section)
    ...
    def test_verify_anywhere_claim(self):
        """The page documents the public verify flag form used by the CLI."""
        content = _read(SITE_DIR / "integrations.html")
...
# ═══════════════════════════════════════════════════════════════════════════
# 14. DOCS PAGE SMOKE TESTS
# ═══════════════════════════════════════════════════════════════════════════


@unittest.skipUnless(SITE_EXISTS, "Website repo not found — set AIIR_SITE_DIR")
class TestDocsPageSmoke(unittest.TestCase):
    """Validate structural claims, version references, and code-block integrity
    on docs.html so drift does not ship to production."""

    def test_four_interface_sections(self):
        """docs.html advertises exactly four interface sections."""
        content = _read(SITE_DIR / "docs.html")
        # The four h3 headings under the "Interfaces" h2
        self.assertIn("💻 CLI", content)
        self.assertIn("🧩 VS Code Extension", content)
        self.assertIn("⚙️ CI", content)
        self.assertIn("🤖 AI Assistants via MCP", content)

    def test_no_legacy_four_interfaces_label(self):
        """The old 'Other Public Interfaces' heading should be gone."""
        content = _read(SITE_DIR / "docs.html")
        self.assertNotIn(
            "Other Public Interfaces",
            content,
            "Legacy 'Other Public Interfaces' heading still present in docs.html",
        )

    def test_version_fallback_matches_aiir(self):
        """Every data-aiir-version fallback text must match __version__."""
        from aiir import __version__

        content = _read(SITE_DIR / "docs.html")
        # Capture (attr_value, text) — attr_value is 'v', 'bare', or ''
        spans = re.findall(
            r'data-aiir-version(?:="([^"]*)")?[^>]*>([^<]+)</span>', content
        )
        self.assertGreater(len(spans), 0, "No data-aiir-version spans found")
        for fmt, fb in spans:
            expected = f"v{__version__}" if fmt == "v" else __version__
            self.assertEqual(
                fb,
                expected,
                f"Fallback version '{fb}' != expected '{expected}'",
            )

    def test_gitlab_legacy_url_pattern(self):
        """Legacy GitLab include URLs must use the v{version} tag pattern."""
        content = _read(SITE_DIR / "docs.html")
        # Strip HTML tags so the URL reconstructs cleanly
        stripped = re.sub(r"<[^>]+>", "", content)
        urls = re.findall(
            r"raw\.githubusercontent\.com/invariant-systems-ai/aiir/v[\d.]+/templates/gitlab-ci\.yml",
            stripped,
        )
        self.assertGreater(len(urls), 0, "No legacy GitLab raw URL found in docs.html")
        for url in urls:
            self.assertNotRegex(url, r"/v\s", f"Space after 'v' in GitLab URL: {url}")

    def test_evidence_levels_section(self):
        """docs.html has an Evidence Levels section with exactly 4 levels."""
        content = _read(SITE_DIR / "docs.html")
        self.assertIn("Evidence Levels", content)
        # Count evidence-level names inside <strong> tags in the tbody
        level_rows = re.findall(
            r"<strong>([^<]+ evidence|[^<]+ context|[^<]+ provenance)</strong>",
            content,
        )
        self.assertEqual(
            len(level_rows),
            4,
            f"Expected 4 evidence levels, found {len(level_rows)}: {level_rows}",
        )

    def test_evidence_level_names(self):
        """Evidence level names match the canonical definitions."""
        content = _read(SITE_DIR / "docs.html")
        expected = [
        ...
        for name in expected:
            self.assertIn(
                name, content, f"Evidence level '{name}' not found in docs.html"
            )

    def test_version_js_included(self):
        """docs.html includes version.js for dynamic version replacement."""
        content = _read(SITE_DIR / "docs.html")
        self.assertIn("version.js", content)

    def test_pricing_zero_deps_scoped_to_cli(self):
        """pricing.html scopes zero-dependencies claim to CLI core."""
        content = _read(SITE_DIR / "pricing.html")
        self.assertIn(
            "Zero runtime dependencies (CLI core)",
            content,
            "pricing.html should scope zero-deps claim to CLI core",
        )
