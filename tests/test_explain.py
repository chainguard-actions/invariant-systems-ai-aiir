...
class TestExplainVerification(unittest.TestCase):
    ...
    def test_signature_verified_with_identity_details(self):
        result = {
            "valid": True,
            "signature": {
                "valid": True,
                "policy": "identity",
                "expected_identity": "https://github.com/example/repo/.github/workflows/release.yml@refs/heads/main",
                ...
                "bundle": {
                    ...
                    "certificate_sha256": "sha256:abc123",
                },
            },
        }
        text = explain_verification(result)
        self.assertIn("Sigstore signature: VERIFIED", text)
        self.assertIn("Expected signer", text)
        self.assertIn("Expected issuer", text)
        self.assertIn("Rekor entries: 1", text)
        self.assertIn("Certificate digest: sha256:abc123", text)

    def test_signature_verified_without_identity_pinning(self):
        result = {
            "valid": True,
            "signature": {
                "valid": True,
                "policy": "any",
                "bundle": {"tlog_entry_count": 0, "entries": []},
            },
        }
        text = explain_verification(result)
        self.assertIn("Signer identity was not pinned", text)

    def test_signature_verified_without_identity_pinning_shows_bundle_details(self):
        result = {
            "valid": True,
            "signature": {
                "valid": True,
                ...
                "bundle": {
                    ...
                    "certificate_sha256": "sha256:def456",
                },
            },
        }
        text = explain_verification(result)
        self.assertIn("Signer identity was not pinned", text)
        self.assertIn(
            "First entry: hashedrekord, logIndex=12345, integrated=2026-04-01T09:03:20Z",
            text,
        )
        self.assertIn("Certificate digest: sha256:def456", text)

    def test_signature_verified_with_sparse_bundle_details(self):
        result = {
            "valid": True,
            "signature": {
                "valid": True,
        ...
        text = explain_verification(result)
        self.assertIn("Expected signer: user@example.com", text)
        self.assertIn("Rekor entries: 1", text)
        self.assertNotIn("First entry:", text)

    def test_signature_verified_with_identity_policy_without_signer_or_bundle(self):
        result = {
            "valid": True,
            "signature": {
                "valid": True,
        ...
        text = explain_verification(result)
        self.assertIn("Sigstore signature: VERIFIED", text)
        self.assertNotIn("Expected signer", text)
        self.assertNotIn("Rekor entries", text)

    def test_signature_failed_explanation(self):
        result = {
            "valid": False,
            "errors": ["content hash mismatch"],
            "signature": {
                "valid": False,
                "error": "missing messageSignature block",
                "errors": [
        ...
        text = explain_verification(result)
        self.assertIn("Sigstore signature: FAILED", text)
        self.assertIn("missing messageSignature block", text)
        self.assertIn("bundle is missing Rekor tlog entries", text)

    def test_signature_failed_without_primary_error_uses_error_list(self):
        result = {
            "valid": False,
            "signature": {
                "valid": False,
                "errors": ["bundle mismatch"],
            },
        }
        text = explain_verification(result)
        self.assertIn("Sigstore signature: FAILED", text)
        self.assertIn("bundle mismatch", text)
