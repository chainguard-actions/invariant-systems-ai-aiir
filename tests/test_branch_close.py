...
class TestLedgerBranchExport(unittest.TestCase):
    ...
    def test_export_empty_ledger(self):
        ...
        try:
            ...
            self.assertEqual(result["evidence_stream"]["events"], [])
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    def test_export_ledger_with_receipts(self):
        ...
        try:
            ...
            self.assertEqual(result["evidence_summary"]["total_receipts"], 1)
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
...
class TestCliBranchGlSastReport(unittest.TestCase):
    ...
    def test_gl_sast_report_written(self):
        """--gl-sast-report writes a SAST JSON file within the cwd.

        Updated by cli-mediums fix: uses a relative path in the current git
        repo cwd rather than changing to a non-git tmpdir.
        """
        import aiir.cli as cli_mod

        # Use a relative path in the git repo cwd where pytest is running.
        sast_path = "gl-sast-branch-test.json"
        sast_full = Path(os.getcwd(), sast_path)
        receipt = {
            "type": "aiir.commit_receipt",
            "commit": {"sha": "a" * 40},
        ...
        try:
            ...
            self.assertEqual(code, 0)
            self.assertTrue(sast_full.exists())
            sast = json.loads(sast_full.read_text())
            self.assertIn("version", sast)
        finally:
            sast_full.unlink(missing_ok=True)
    ...
    def test_gl_sast_report_rejects_relative_traversal(self):
        ...
        try:
            ...
            self.assertEqual(code, 1)
            # Updated to match the improved error message from the cli-mediums fix:
            # the message now says "working directory" instead of "within the project".
            self.assertIn("working directory", captured_err.getvalue())
        finally:
...
class TestVerifyReleaseBranchComputeCoverage(unittest.TestCase):
    ...
    def test_receipt_with_non_dict_commit(self):
        ...
        Updated to assert the fixed behavior (D1): only type=='aiir.commit_receipt'
        receipts count.  The second receipt has the correct type and a valid SHA so
        it counts; the first has a non-dict commit and is skipped even with the type.
        """
        ...
        result = _compute_coverage(
            ["abc123"],
            [
                {"type": "aiir.commit_receipt", "commit": "not-a-dict"},
                {"type": "aiir.commit_receipt", "commit": {"sha": "abc123"}},
            ],
        )
    ...
    def test_receipt_with_empty_sha(self):
        ...
        Updated to assert the fixed behavior (D1): only type=='aiir.commit_receipt'
        receipts count.  The second receipt has the correct type and a valid SHA so
        it counts; the first has an empty sha and is skipped.
        """
        from aiir._verify_release import _compute_coverage
        ...
        result = _compute_coverage(
            ["abc123"],
            [
                {"type": "aiir.commit_receipt", "commit": {"sha": ""}},
                {"type": "aiir.commit_receipt", "commit": {"sha": "abc123"}},
            ],
        )
        self.assertEqual(result["receipts_found"], 1)
...
class TestCliBranchQuietMode(unittest.TestCase):
    ...
    def test_sast_report_quiet(self):
        ...
        Updated by cli-mediums fix: uses a relative path in the git repo cwd
        rather than changing to a non-git tmpdir.
        """
        import aiir.cli as cli_mod

        # Use a relative path in the git repo cwd where pytest is running.
        sast_path = "gl-sast-quiet-branch-test.json"
        sast_full = Path(os.getcwd(), sast_path)
        try:
            with (
                patch("aiir.cli.generate_receipt", return_value=self._make_receipt()),
