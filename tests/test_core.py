...
class TestPrettyAndSummaryGuard(unittest.TestCase):
    ...
    def test_summary_includes_signing_counts(self):
        """format_github_summary should show signed and unsigned receipt totals."""
        receipts = [
            {
                "receipt_id": "signed-receipt",
                "commit": {"sha": "abcdef1234567890", "subject": "signed"},
                "ai_attestation": {"is_ai_authored": False},
                "extensions": {
                    "sigstore_bundle": {
                        "mediaType": "application/vnd.dev.sigstore.bundle.v0.3+json",
                        "verificationMaterial": {"certificate": {"rawBytes": "QQ=="}},
                        "messageSignature": {"signature": "Zm9v"},
                    }
                },
            },
            {
                "receipt_id": "unsigned-receipt",
                "commit": {"sha": "1234567890abcdef", "subject": "unsigned"},
                "ai_attestation": {"is_ai_authored": False},
            },
        ]
        result = cli.format_github_summary(receipts)
        self.assertIn("**1** signed", result)
        self.assertIn("**1** unsigned", result)
