...
from aiir._agent_receipt import (  # noqa: E402
    build_agent_receipt,
...
# ═══════════════════════════════════════════════════════════════════════
# PROPERTY 16: Agent-receipt round-trip and single-field tamper detection
# ═══════════════════════════════════════════════════════════════════════

# Strategy: build valid inputs for build_agent_receipt
_agent_kind = st.text(
    alphabet=st.sampled_from(list(string.ascii_lowercase + "_-")),
    min_size=1,
    max_size=40,
...
_agent_action_kind = st.sampled_from(["edit", "run", "read", "test", "build", "review"])
_agent_decision = st.sampled_from(["allowed", "denied", "not_evaluated"])
_agent_surface = st.one_of(
    st.none(),
    st.sampled_from(["cli", "ide", "ci", "mcp"]),
)
# Only printable ASCII (0x20-0x7E) so _strip_terminal_escapes never empties the string.
_agent_printable = st.text(
    alphabet=st.sampled_from(
        list(
...
_agent_session = st.one_of(st.none(), _agent_printable.map(lambda s: s[:40]))
_agent_intent = st.one_of(st.none(), _agent_printable.map(lambda s: s[:100]))
_agent_summary = st.one_of(st.none(), _agent_printable.map(lambda s: s[:300]))

_build_agent_receipt_st = st.builds(
    build_agent_receipt,
    actor_kind=_agent_kind,
    actor_tool_name=_agent_tool,
    action_kind=_agent_action_kind,
    actor_tool_surface=_agent_surface,
    actor_session_ref=_agent_session,
    action_intent=_agent_intent,
    action_summary=_agent_summary,
    policy_decision=_agent_decision,
)


class TestAgentReceiptProperties:
    """Property-based tests for the agent-receipt build/verify contract."""

    @given(receipt=_build_agent_receipt_st)
    @settings(
        max_examples=300,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors],
    )
    def test_build_verify_round_trip(self, receipt: dict):
        """Every receipt produced by build_agent_receipt must self-verify as valid."""
        result = verify_agent_receipt(receipt)
        assert result["valid"], (
            f"Self-verification failed for built receipt: errors={result.get('errors')}"
        )
    ...
    def test_build_verify_json_round_trip(self, receipt: dict):
        """Receipt survives JSON serialization and still verifies valid."""
        import json as _json

        serialized = _json.dumps(receipt, sort_keys=True)
        deserialized = _json.loads(serialized)
        result = verify_agent_receipt(deserialized)
        assert result["valid"], (
            f"JSON round-trip broke agent receipt verification: errors={result.get('errors')}"
        )

    @given(receipt=_build_agent_receipt_st, new_kind=_agent_action_kind)
    @settings(
        max_examples=200,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors],
    )
    def test_tamper_action_kind_detected(self, receipt: dict, new_kind: str):
        """Mutating action.kind must make verify_agent_receipt return valid=False."""
        import copy

        assume(new_kind != receipt["action"]["kind"])
        tampered = copy.deepcopy(receipt)
        tampered["action"]["kind"] = new_kind
        result = verify_agent_receipt(tampered)
        assert not result["valid"], (
            f"Tampered action.kind '{new_kind}' was not detected"
        )

    @given(receipt=_build_agent_receipt_st, new_tool=_agent_tool)
    @settings(
        max_examples=200,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors],
    )
    def test_tamper_actor_tool_name_detected(self, receipt: dict, new_tool: str):
        """Mutating actor.tool.name must invalidate the receipt."""
        import copy

        assume(new_tool.strip() and new_tool != receipt["actor"]["tool"]["name"])
        tampered = copy.deepcopy(receipt)
        tampered["actor"]["tool"]["name"] = new_tool
        result = verify_agent_receipt(tampered)
        assert not result["valid"], (
            f"Tampered actor.tool.name '{new_tool}' was not detected"
        )

    @given(receipt=_build_agent_receipt_st, new_decision=_agent_decision)
    @settings(
        max_examples=200,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.differing_executors],
    )
    def test_tamper_policy_decision_detected(self, receipt: dict, new_decision: str):
        """Mutating policy.decision must invalidate the receipt."""
        import copy

        assume(new_decision != receipt["policy"]["decision"])
        tampered = copy.deepcopy(receipt)
        tampered["policy"]["decision"] = new_decision
        result = verify_agent_receipt(tampered)
        assert not result["valid"], (
            f"Tampered policy.decision '{new_decision}' was not detected"
        )
    ...
    def test_tamper_content_hash_detected(self, receipt: dict):
        """Replacing proof.content_hash with a wrong value must invalidate receipt."""
        import copy

        tampered = copy.deepcopy(receipt)
        original_hash = tampered["proof"]["content_hash"]
        # Flip one hex character in the digest portion
        prefix = "sha256:"
        hex_part = original_hash[len(prefix) :]
        flipped = ("a" if hex_part[0] != "a" else "b") + hex_part[1:]
        tampered["proof"]["content_hash"] = prefix + flipped
        assume(tampered["proof"]["content_hash"] != original_hash)
        result = verify_agent_receipt(tampered)
        assert not result["valid"], "Tampered content_hash was not detected"
    ...
    def test_arbitrary_input_never_raises(self, data):
        """verify_agent_receipt must never raise on arbitrary input (only valid=False)."""
        try:
            result = verify_agent_receipt(data)  # type: ignore[arg-type]
        except Exception as exc:  # noqa: BLE001
            raise AssertionError(
                f"verify_agent_receipt raised {type(exc).__name__}: {exc}"
            ) from exc
        assert isinstance(result, dict), "Must return a dict"
        assert "valid" in result, "Result must have 'valid' key"
        assert result["valid"] is False, (
            f"Non-receipt input must not verify as valid, got: {result}"
        )
