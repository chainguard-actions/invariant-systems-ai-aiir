...
class TestHostileAngle3SchemaAttacks(unittest.TestCase):
    ...
    def test_tool_uri_forgery(self):
        """Malformed tool URIs are rejected; valid third-party https URIs are accepted.

        C3 relaxed provenance.tool from the old hardcoded AIIR-repo prefix to any
        non-empty https URI without whitespace or control characters.  Third-party
        generators such as 'https://github.com/evil-org/evil-tool@1.0.0' are now
        VALID — tool-string shape is NOT the integrity boundary.  The real security
        boundary is content_hash + signature: if those pass, the receipt is genuine
        regardless of which tool produced it.

        This test proves that the schema still rejects genuinely malformed URIs:
        - non-https scheme (http://) violates the https requirement
        - embedded newlines/control chars violate the shape predicate

        A test that merely asserts a third-party https URI is rejected would be
        asserting the OLD (reverted) behaviour and must NOT be restored.
        """
        # A valid third-party https URI is now ACCEPTED (not forged in any meaningful
        # sense — it has the right shape; integrity rests on content_hash + signature).
        r_valid = _make_valid_receipt()
        r_valid["provenance"]["tool"] = "https://github.com/evil-org/evil-tool@1.0.0"
        valid_errors = schema.validate_receipt_schema(r_valid)
        tool_errors_valid = [e for e in valid_errors if "tool" in e.lower()]
        self.assertEqual(
            len(tool_errors_valid),
            0,
            "A well-formed third-party https tool URI must be accepted under C3",
        )

        # A non-https URI is still rejected — the https scheme is the real boundary.
        r_http = _make_valid_receipt()
        r_http["provenance"]["tool"] = "http://evil.com/tool"
        http_errors = schema.validate_receipt_schema(r_http)
        tool_errors_http = [e for e in http_errors if "tool" in e.lower()]
        self.assertTrue(
            len(tool_errors_http) > 0,
            "Non-https tool URI 'http://evil.com/tool' must be rejected by schema",
        )

        # A URI with embedded whitespace/control chars is also rejected.
        r_ctrl = _make_valid_receipt()
        r_ctrl["provenance"]["tool"] = "https://x\n@1.0"
        ctrl_errors = schema.validate_receipt_schema(r_ctrl)
        tool_errors_ctrl = [e for e in ctrl_errors if "tool" in e.lower()]
        self.assertTrue(
            len(tool_errors_ctrl) > 0,
            "Tool URI with embedded newline 'https://x\\n@1.0' must be rejected by schema",
        )
...
class TestHostileAngle5MCPProtocol(unittest.TestCase):
    ...
    def test_initialize_returns_protocol_version(self):
        ...
        WS-E downgraded the advertised MCP PROTOCOL_VERSION to '2024-11-05'
        because this server does not implement the batch-request support that
        '2025-03-26' mandates.  Honest advertising — clients negotiating the
        newer version would otherwise enable features this server cannot handle.
        """
