...
class TestActionYmlExists(unittest.TestCase):
    ...
    def test_action_yml_has_required_outputs(self):
        ...
        self.assertIn("signed_receipt_count:", content)
        self.assertIn("unsigned_receipt_count:", content)
...
class TestSignstoreInstall(unittest.TestCase):
    """Verify sigstore install is hash-pinned."""

    def test_sigstore_version_pinned(self):
        """Sigstore install uses --require-hashes from pinned requirements file."""
        content = ACTION_YML.read_text()
        self.assertIn("--require-hashes", content)
        self.assertIn(".github/requirements/sigstore.txt", content)
