...
class TestGitLabApiRequestHardening(unittest.TestCase):
    ...
    WS-E: _gitlab_api_request MUST reject any non-HTTPS base URL before
    constructing a request.  Cleartext http:// and non-HTTP schemes (file://,
    ftp://, etc.) all expose the authentication token; the implementation
    refuses them and the message must reference HTTPS.
    """

    def test_rejects_http_scheme(self):
        """http:// base_url is rejected — cleartext would expose auth token."""
        ...
        # Must mention HTTPS (or non-HTTPS) so the rejection reason is clear.
        self.assertTrue(
            "https" in msg.lower() or "non-https" in msg.lower(),
            f"Expected HTTPS-required wording in error, got: {msg!r}",
        )

    def test_rejects_file_scheme(self):
        """file:// base_url is rejected before any request is constructed."""
        from aiir._gitlab import _gitlab_api_request
    ...
    def test_rejects_ftp_scheme(self):
        """ftp:// base_url is rejected — only HTTPS is permitted."""
        from aiir._gitlab import _gitlab_api_request
        ...
        with self.assertRaises(RuntimeError) as ctx:
            _gitlab_api_request(
                ...
                "/projects/1/notes",
                body={"body": "x"},
                api_url="ftp://attacker.example.com",
                token="glpat_test",
            )
        msg = str(ctx.exception)
        self.assertTrue(
