...
class TestEvaluateLedgerPolicy(unittest.TestCase):
    ...
    def test_unsigned_receipts_exceeded_strict(self):
        index = {
            "receipt_count": 2,
        ...
        self.assertFalse(passed)
        self.assertIn("FAIL", msg)
        self.assertIn("max_unsigned_receipts", [v.rule for v in violations])

    def test_unsigned_receipts_equal_to_limit_passes(self):
        index = {
            "receipt_count": 2,
            "signed_receipt_count": 1,
            "unsigned_receipt_count": 1,
        ...
        policy = {"max_unsigned_receipts": 1, "enforcement": "hard-fail"}
        passed, _, violations = evaluate_ledger_policy(index, policy)
        self.assertTrue(passed)
        self.assertEqual(violations, [])

    def test_negative_max_unsigned_receipts_skips_check(self):
        index = {
            "receipt_count": 5,
            "signed_receipt_count": 0,
            "unsigned_receipt_count": 5,
            "ai_commit_count": 0,
            "ai_percentage": 0.0,
        }
        policy = {"max_unsigned_receipts": -1, "enforcement": "hard-fail"}
    ...
    def test_unsigned_violation_message_has_counts(self):
        index = {
            "receipt_count": 4,
            "signed_receipt_count": 1,
        ...
        self.assertEqual(violations[0].rule, "max_unsigned_receipts")
