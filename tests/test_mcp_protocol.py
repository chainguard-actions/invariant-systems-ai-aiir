...
class TestServeStdioMalformedInput(unittest.TestCase):
    ...
    def test_non_dict_json_returns_error(self):
        """A JSON array (batch request) is rejected with a -32600 error.

        WS-E hardening: a top-level JSON array is now detected as an
        unsupported batch request and returns 'Batch requests are not
        supported' rather than the generic 'expected JSON object' message.
        This test asserts the new, correct behavior — it will fail if the
        batch-detection branch is reverted.
        """
        responses = _run_server(
            [
                json.dumps([1, 2, 3]),
        ...
        # First response is a -32600 error for the batch array
        err = responses[0]
        self.assertIn("error", err)
        self.assertEqual(err["error"]["code"], -32600)
        self.assertIn("Batch requests are not supported", err["error"]["message"])
    ...
    def test_main_module_calls_cli_main(self):
        ...
        with patch("aiir.cli.main", return_value=0):
            with self.assertRaises(SystemExit) as ctx:
                exec(
                    compile(
                        Path("aiir/__main__.py").read_text(),
                        "aiir/__main__.py",
                        "exec",
                    )
                )
