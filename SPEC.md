...
> **Normative (CBOR)**: CBOR map keys in the canonical envelope MUST be ordered
> bytewise (lexicographically) over their encoded CBOR byte sequences, as required
> by RFC 8949 section 4.2.1 core deterministic encoding. For all AIIR receipt map
> keys (which are short text strings of a single CBOR major type), this ordering
> is equivalent to length-first order in practice, but the bytewise definition
> is authoritative for conformance testing.
...
### 3.1 Privacy and Email Redaction

Author and committer email addresses are included in the hashed core by default.
This means they contribute to `content_hash` and `receipt_id`. Under
`--redact-emails`, the reference implementation replaces email addresses with
a per-ledger HMAC-SHA256 pseudonym in the form
`redacted+<16-hex-chars>@users.noreply.aiir`, where the 16-hex-chars are the
first 16 hexadecimal characters of HMAC-SHA256(per-ledger-salt, email).

> **Normative**: Email redaction is **pseudonymous-per-ledger**, NOT anonymous.
> The same email address redacted under the same ledger salt produces the same
> pseudonym, enabling per-ledger deduplication. Different ledger salts produce
> different pseudonyms for the same email, providing cross-ledger
> non-linkability. The salt is stored in `.aiir/config.json` (0600) and is
> required to reproduce pseudonymized receipt IDs. See [PRIVACY.md](PRIVACY.md)
> for the full data-handling disclosure, GDPR Art. 17 considerations, and
> backup guidance.
...
- **message_hash**: `"sha256:" + hex(SHA-256(body.encode("utf-8")))` where
  `body` is the **full raw commit message** as produced by `git log --format=%B`,
  that is, the subject line, the blank separator, the body, and any trailers,
  with leading and trailing whitespace stripped. The subject line IS included.
  Independent verifiers can reproduce this with:
  `git log -1 --format=%B <sha> | python3 -c "import sys,hashlib; d=sys.stdin.read().strip(); print('sha256:'+hashlib.sha256(d.encode()).hexdigest())"`

- **diff_hash**: `"sha256:" + hex(SHA-256(diff_bytes))` where `diff_bytes` is
  the raw standard-output of the following canonical git invocation:
  ...
  git --no-optional-locks \
    ...
    -U3 --indent-heuristic --find-renames \
    <parent> <sha>
  ```

  Every pinned `-c` value equals git's compiled default for the common case.
  This means `diff_hash` is identical to what an un-configured git would produce
  for ASCII-path commits, while remaining reproducible under non-default user
  config. `core.quotepath=false` canonicalises non-ASCII file paths (no deployed
  receipt has non-ASCII paths, so this is forward-looking only).

  For root commits (no parent), the diff is computed against the empty tree
  using `git hash-object -t tree /dev/null` as the parent ref.
...
> **Note:** The following legacy aliases appeared in schema versions ≤ 1.0.13
> and are accepted by the reference validator for backward compatibility but
> SHOULD NOT be produced by new implementations:
> `ai_generated`, `ai-generated`, `ai-assisted`, `bot-generated`.
> Use the canonical four values (`human`, `ai_assisted`, `bot`, `ai+bot`) for
> all new receipts. Use `ai+bot` for commits with both AI and bot signals.
...
| `tool` | string | ✅ | Tool-identity URI. MUST be an https URI. Producers SHOULD include a version (e.g. `https://github.com/invariant-systems-ai/aiir@1.6.0`); third-party generators use their own https URI. |
...
To verify a receipt, an implementation MUST execute the following steps in
order. The receipt is VALID if and only if BOTH hash comparisons in steps 6–7
succeed. Structural/JSON-Schema validation is RECOMMENDED for diagnostics and
MAY feed policy decisions, but MUST NOT alter the validity verdict.

1. **Type check**: Verify `type == "aiir.commit_receipt"`.

2. **Schema prefix check**: Verify `schema` is a string beginning with the
   prefix `"aiir/"`. Recognized values: `aiir/commit_receipt.v1`,
   `aiir/commit_receipt.v2`.

3. **Version format check**: Verify `version` matches
   `^[0-9]+\.[0-9]+\.[0-9]+([.+\-][0-9a-zA-Z.+\-]*)?$` (SemVer).

4. **Core extraction**: Extract
   `core = {k: v for k, v in receipt.items() if k in CORE_KEYS}`.

5. **Canonical encoding**: Compute `core_json = canonical_json(core)`.

6. **Hash computation**: Compute:
   - `expected_hash = "sha256:" + hex(SHA-256(core_json.encode("utf-8")))`
   - `expected_id = "g1-" + hex(SHA-256(core_json.encode("utf-8")))[:32]`

7. **Constant-time comparison**: Compare `expected_hash` against
   `receipt["content_hash"]` and `expected_id` against
   `receipt["receipt_id"]` using constant-time comparison
   (e.g., `hmac.compare_digest`).

> **Advisory note**: Structural JSON-Schema validation (checking required
> fields, types, and patterns) is RECOMMENDED and errors SHOULD be reported
> via `schema_errors`, but the validity verdict depends solely on the hash
> comparison in step 7. Unknown top-level fields outside CORE_KEYS are
> tolerated and ignored (forward compatibility). For `aiir/commit_receipt.v2`,
> `commit.tree_sha` and `commit.parent_shas` SHOULD be present; their absence
> generates an advisory schema error but does not flip the verdict.
...
| **Tier 2 — Signed** | `sign: true` (default in GitHub Action) | Tamper-**evident** (Sigstore signature + transparency log) | Signer identity bound via OIDC + transparency log | CI/CD compliance, SOC 2 evidence, audit-grade provenance |
| **Tier 3 — Enveloped** | in-toto Statement wrapper (§14.2) | Tamper-**evident** + envelope integrity | Predicate + subject bound to signer | Supply-chain attestation, SLSA provenance, cross-system verification |
...
The in-toto predicate type URI for `aiir/commit_receipt.v2` is:

```text
https://invariantsystems.io/predicates/aiir/commit_receipt/v2
```

This URI identifies `aiir/commit_receipt.v2` predicates within
...
Legacy `aiir/commit_receipt.v1` receipts use the matching historical predicate
type URI:
...
Implementations MUST emit the predicate type URI that matches the wrapped
receipt's `schema` field.
