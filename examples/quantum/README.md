...
Record exactly what ran, where, and when, and prove it wasn't tampered with,
...
A: Yes, `pip install aiir`. AIIR has zero dependencies (stdlib-only), so
...
each other, so use both for full provenance.
