...
from aiir._agent_receipt import is_agent_receipt, verify_agent_receipt
from aiir._commitment import is_commitment_receipt, verify_commitment_receipt
...
def verify_receipt(receipt: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if is_commitment_receipt(receipt):
        return verify_commitment_receipt(receipt)

    # Agent receipts (aiir/agent_receipt.v0.1) are keyed by contract_version,
    # not type/schema, so route them before the commit-receipt checks below.
    if is_agent_receipt(receipt):
        return verify_agent_receipt(receipt)
    ...
    _KNOWN_RECEIPT_TYPES = {"aiir.commit_receipt", "aiir.review_receipt"}
    if rtype not in _KNOWN_RECEIPT_TYPES:
    ...
    # For commit receipts: type, schema, version, commit, ai_attestation, provenance.
    # For review receipts: type, schema, version, reviewed_commit, reviewer,
    #   review_outcome, comment, provenance (comment is optional but in core when present).
    # Both types use the same content-hash mechanism; AIIR_CORE_KEYS in _receipt.py
    # covers both by including all possible core-field names.
...
def verify_receipt_file(filepath: str) -> Dict[str, Any]:
    ...
    if isinstance(data, list):
        ...
        # Auto-detect inference/research arrays by the leading element's
        # signature, then let the dedicated verifier validate every entry and
        # report per-entry errors.  This is fail-closed: a mixed array that is
        # misrouted still fails verification (the chain verifier rejects the
        # non-conforming entries) rather than silently passing.
        if data and is_inference_receipt(data[0]):
            return verify_inference_chain(data)
        if data and is_research_evidence_receipt(data[0]):
        ...
        # Auto-detect inference receipts by their field signature
        if is_inference_receipt(data):
            return verify_inference_receipt(data)
        if is_research_evidence_receipt(data):
...
def verify_receipt_ledger_file(filepath: str) -> Dict[str, Any]:
    """Load and verify a JSONL receipt ledger.

    Each non-empty line must be a receipt JSON object. The aggregate result is
    valid only when every parsed receipt verifies and no line-level parse errors
    occur. Empty ledgers are treated as valid with zero receipts.
    """
    path = Path(filepath)
    if not path.exists():
        return {"valid": False, "error": f"File not found: {filepath}"}
    if path.is_symlink():
        return {
            "valid": False,
            "error": f"Receipt ledger is a symlink (refusing to verify): {filepath}",
        }
    try:
        file_size = path.stat().st_size
    except OSError as e:  # pragma: no cover
