...
def _derive_unsigned_receipt_count(index: Dict[str, Any]) -> Optional[int]:
    explicit_unsigned = _coerce_int(index.get("unsigned_receipt_count"))
    if explicit_unsigned is not None:
        return max(explicit_unsigned, 0)

    total = _coerce_int(index.get("receipt_count"))
    if total is None:
        total = 0

    signed_count = _coerce_int(index.get("signed_receipt_count"))
    if signed_count is not None:
        return max(total - signed_count, 0)
    ...
    if isinstance(commits, dict) and commits:
        ...
        for entry in commits.values():
            ...
            if bool(entry.get("signed")):
                signed_true += 1
        if signed_known:
            return max(total - signed_true, 0)
...
def evaluate_receipt_policy(
    ...
    *,
    ...
    # require_signing is a hard gate — severity is always error regardless of
    # enforcement level (C5.4: soft-fail may warn on advisory items but MUST
    # fail a tripped require_* gate).
    ...
    # require_provenance_repo is a hard gate — severity is always error (C5.4).
    ...
    # 4. Schema validity
    # require_schema_valid is a hard gate — severity is always error (C5.4).
...
def evaluate_ledger_policy(
    ...
    # Max unsigned receipts
    max_unsigned = _coerce_int(policy.get("max_unsigned_receipts"))
    if max_unsigned is not None and max_unsigned >= 0:
        unsigned_count = _derive_unsigned_receipt_count(index)
        total = _coerce_int(index.get("receipt_count")) or 0
        if unsigned_count is not None and unsigned_count > max_unsigned:
            violations.append(
                PolicyViolation(
                    rule="max_unsigned_receipts",
                    message=(
                        f"Unsigned receipt count {unsigned_count} exceeds policy maximum {max_unsigned}"
                        f" ({unsigned_count}/{total} receipts unsigned)"
                    ),
                    severity="error",
                    remediation=(
                        "Regenerate receipts with signing enabled, or raise "
                        "max_unsigned_receipts in .aiir/policy.json."
                    ),
                )
            )
