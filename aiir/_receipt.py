...
from aiir._commitment import (
    COMMITMENT_CORE_KEYS,
...
def _sanitize_tool_context(
    ...
    """Sanitize receipt tool-context extension entries.

    Tool context captures companion IDE or workflow surfaces that were present
    when the receipt was generated. Unlike ``agent_attestation``, this does not
    imply AI authorship by itself; it is an auxiliary context channel stored in
    ``extensions.tool_context``.
    """
    if tool_context is None:
        return []

    items = tool_context if isinstance(tool_context, list) else [tool_context]
        ...
        for key in _TOOL_CONTEXT_KEYS:
            ...
            if key == "evidence":
                ...
                for entry in value[:10]:
                    sanitized = _strip_terminal_escapes(str(entry))[:120]
                    if sanitized:
            ...
            sanitized = _strip_terminal_escapes(str(value))[:200]
            if sanitized:
                clean[key] = sanitized
        if clean:
...
# Allowed top-level keys in editor_provenance — prevents arbitrary data injection.
# Values that are strings are sanitized; nested dicts/lists are stripped.
_EDITOR_PROVENANCE_KEYS = frozenset(
    {
        "schema",  # e.g. "aiir/editor_provenance.v1"
        "mode",  # e.g. "provable", "basic"
        "toolId",  # e.g. "aiir-vscode"
        "recordCount",  # integer (coerced to str for safety)
        "chainHead",  # content hash hex string
        "merkleRoot",  # merkle root hex string
    }
)


def _sanitize_editor_provenance(
    provenance: Optional[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """Sanitize editor_provenance to the same standard as agent_attestation.

    Only known top-level string/int keys are passed through; unknown keys and
    nested structures (e.g. ``records``) are preserved only if they are lists
    of dicts that themselves only contain string values, to prevent arbitrary
    data injection.  All string values are stripped of terminal escapes and
    capped at safe lengths.
    """
    if not isinstance(provenance, dict):
        return None
    clean: Dict[str, Any] = {}
    for key in _EDITOR_PROVENANCE_KEYS:
        ...
        if s:
            clean[key] = s
    # Preserve "records" as a sanitized list of dicts (string values only).
    records_raw = provenance.get("records")
    if isinstance(records_raw, list):
        records_clean = []
        for rec in records_raw[:100]:
            if not isinstance(rec, dict):
                continue
            rec_clean: Dict[str, str] = {}
            for k, v in rec.items():
                if isinstance(k, str) and not isinstance(v, (dict, list)):
                    k_safe = _strip_terminal_escapes(str(k))[:100]
                    v_safe = _strip_terminal_escapes(str(v))[:500]
                    if k_safe:
                        rec_clean[k_safe] = v_safe
            if rec_clean:
                records_clean.append(rec_clean)
        if records_clean:
            clean["records"] = records_clean
...
def build_commit_receipt(
    ...
    editor_provenance: Optional[Dict[str, Any]] = None,
    ...
    if redact_emails:
        author_email = _redacted_email(author_email, redaction_salt)
        committer_email = _redacted_email(committer_email, redaction_salt)
    ...
    receipt = {
        **receipt_core,
        ...
        "extensions": {
            ...
            **(
                ({"editor_provenance": _sanitize_editor_provenance(editor_provenance)})
                if editor_provenance
                else {}
            ),
            **(
                ({"tool_context": _sanitize_tool_context(tool_context)})
                if tool_context
                else {}
            ),
        },
...
def _extract_receipt_core(receipt: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if receipt.get("type") == COMMITMENT_RECEIPT_TYPE:
        return {k: receipt[k] for k in COMMITMENT_CORE_KEYS if k in receipt}
...
def _redacted_email(email: str, salt: Optional[str] = None) -> str:
    """Return a pseudonymous email-shaped token derived from HMAC-SHA256.

    The token is per-ledger-salt unlinkable: two ledgers with different salts
    produce different tokens for the same email.  When *salt* is ``None``
    (e.g. library callers without a ledger), a fixed, PUBLIC fallback constant
    is used so callers still get a deterministic pseudonym rather than crashing.
    The fallback path is NOT unlinkable: because the constant is public, anyone
    can recompute the token for a guessed email (a confirm/deny oracle, and
    dictionary-reversible). Real cross-ledger unlinkability requires the secret
    per-ledger salt — the CLI ``--redact-emails`` path always supplies it (see
    :func:`aiir._ledger._ensure_redaction_salt`); library callers that need
    unlinkability MUST pass a secret ``salt``.

    Args:
        email: The raw email address to pseudonymize.
        salt:  256-bit hex string from ``.aiir/config.json``, or ``None``.

    Returns:
        ``"redacted+<16hex>@users.noreply.aiir"`` or the blank-identity
        fallback ``"redacted@users.noreply.aiir"``.
    """
    normalized = _strip_terminal_escapes(email).strip().lower()
    if not normalized:
        return "redacted@users.noreply.aiir"
    digest = _hmac16(salt, normalized)
    return f"redacted+{digest}@users.noreply.aiir"
...
# Predicate type URIs registered for AIIR commit receipts.
# Format: https://in-toto.io/attestation/v1#predicateType
INTOTO_PREDICATE_TYPES = {
    ...
    COMMITMENT_RECEIPT_SCHEMA_VERSION: "https://invariantsystems.io/predicates/aiir/commitment_receipt/v1",
}

# Current default predicate type for newly generated receipts.
INTOTO_PREDICATE_TYPE = INTOTO_PREDICATE_TYPES[RECEIPT_SCHEMA_VERSION]


def _predicate_type_for_receipt(receipt: Dict[str, Any]) -> str:
    """Return the in-toto predicate type URI matching the wrapped receipt schema."""
    schema = str(receipt.get("schema") or RECEIPT_SCHEMA_VERSION)
    return INTOTO_PREDICATE_TYPES.get(schema, INTOTO_PREDICATE_TYPE)
...
def wrap_in_toto_statement(receipt: Dict[str, Any]) -> Dict[str, Any]:
    ...
    full AIIR receipt including its content hash.  Downstream verifiers
        can match on `predicateType` to route attestations. Legacy
        `aiir/commit_receipt.v1` receipts retain the matching `/v1`
        predicate URI when wrapped.
    ...
    if receipt.get("type") == COMMITMENT_RECEIPT_TYPE:
        ...
        for artifact in artifacts:
            ...
            subjects.append(
                {
                    "name": _strip_terminal_escapes(
                        str(artifact.get("ref", "artifact"))
                    )[:200],
        ...
        if not subjects:
            ...
            subjects = [
                {
                    "name": _strip_terminal_escapes(
                        str(subject.get("name", "commitment"))
                    )[:200],
                    "digest": {
                        "sha256": str(receipt.get("content_hash", "sha256:")).split(
                            ":", 1
                        )[1]
                    },
                }
...
def format_receipt_pretty(receipt: Dict[str, Any], signed: str = "none") -> str:
    ...
    if receipt.get("type") == COMMITMENT_RECEIPT_TYPE:
        ...
        name = _strip_terminal_escapes(str(subject.get("name", "unknown")))[:80]
        kind = _strip_terminal_escapes(str(subject.get("kind", "unknown")))[:40]
        summary = _strip_terminal_escapes(str(statement.get("summary", "")))[:96]
        ...
        lines = [
            f"{tl}{hl} Commitment: {receipt_id}",
            f"{vl}  Subject: {name}",
            f"{vl}  Kind:    {kind}",
            f"{vl}  Summary: {summary}",
            f"{vl}  Items:   {len(artifacts)} artifact(s)",
...
def format_receipt_detail(receipt: Dict[str, Any], signed: str = "none") -> str:
    ...
    if receipt.get("type") == COMMITMENT_RECEIPT_TYPE:
        ...
        sec = f"{vl}{'─' * 44}"
        lines = [
            f"{tl}{hl} Commitment: {_strip_terminal_escapes(str(receipt.get('receipt_id', 'unknown')))}",
            sec,
            f"{vl}  Type:    {_strip_terminal_escapes(str(receipt.get('type', '')))}",
            f"{vl}  Schema:  {_strip_terminal_escapes(str(receipt.get('schema', '')))}",
            f"{vl}  Version: {_strip_terminal_escapes(str(receipt.get('version', '')))}",
            sec,
            f"{vl}  Subject:  {_strip_terminal_escapes(str(subject.get('name', '')))}",
            f"{vl}  Kind:     {_strip_terminal_escapes(str(subject.get('kind', '')))}",
            f"{vl}  Summary:  {_strip_terminal_escapes(str(statement.get('summary', '')))[:120]}",
            sec,
            f"{vl}  Items:    {len(artifacts)} artifact(s)",
        ]
        for artifact in artifacts[:10]:
            if not isinstance(artifact, dict):
                continue
            lines.append(
                f"{vl}  - {_strip_terminal_escapes(str(artifact.get('type', 'artifact')))} "
                f"{_strip_terminal_escapes(str(artifact.get('ref', 'unknown')))[:80]}"
            )
            lines.append(
                f"{vl}    digest: {_strip_terminal_escapes(str(artifact.get('digest', '')))[:96]}"
            )
        if len(artifacts) > 10:
            lines.append(f"{vl}  ... and {len(artifacts) - 10} more")
        lines.append(sec)
        lines.append(
            f"{vl}  Repo:     {_strip_terminal_escapes(str(provenance.get('repository', '')))}"
        )
        lines.append(
            f"{vl}  Tool:     {_strip_terminal_escapes(str(provenance.get('tool', '')))}"
        )
        lines.append(
            f"{vl}  Generator:{' ' if provenance.get('generator') else ''}{_strip_terminal_escapes(str(provenance.get('generator', '')))}"
        )
        lines.append(sec)
        lines.append(
            f"{vl}  Hash:    {_strip_terminal_escapes(str(receipt.get('content_hash', '')))}"
        )
        lines.append(
            f"{vl}  Time:    {_strip_terminal_escapes(str(receipt.get('timestamp', '')))}"
        )
        lines.append(f"{vl}  Signed:  {_strip_terminal_escapes(signed)}")
        if extensions:
            lines.append(sec)
            for i, (key, value) in enumerate(extensions.items()):
                if i >= 10:
                    lines.append(f"{vl}  ... and {len(extensions) - 10} more")
                    break
                lines.append(
                    f"{vl}  ext.{_strip_terminal_escapes(str(key))[:40]}: "
                    f"{_strip_terminal_escapes(str(value))[:80]}"
                )
...
def write_receipt(
    ...
    if output_dir:
        ...
        # If this exact receipt already exists, verify it matches before deduping.
        # A content-hash collision (different receipt_id in existing file) would
        # silently lose data; surface it as a conflict instead.
        if filepath.exists():
            try:
                with open(str(filepath), "r", encoding="utf-8") as _existing_f:
                    _existing = json.loads(_existing_f.read())
                _existing_rid = _existing.get("receipt_id")
                _new_rid = receipt.get("receipt_id")
                if _existing_rid != _new_rid:
                    raise ValueError(
                        f"Receipt conflict: file {filepath.name!r} already exists "
                        f"with receipt_id={_existing_rid!r}, "
                        f"but new receipt has receipt_id={_new_rid!r}. "
                        f"Refusing to overwrite — delete the file to regenerate."
                    )
            except (json.JSONDecodeError, OSError):
                # Unreadable/corrupt existing file — overwrite it safely below
                # (os.open with O_EXCL would fail; fall through to write path)
                pass
            else:
                # Identical receipt_id — safe to dedup
                if not cbor_path.exists():
                    _write_bytes_atomic(
                        cbor_path, _canonical_receipt_cbor_bytes(receipt)
                    )
                return str(filepath)
