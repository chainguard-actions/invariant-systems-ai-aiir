...
def build_commit_receipt(
    ...
    if redact_emails:
        author_email = _redacted_email(author_email)
        committer_email = _redacted_email(committer_email)
    ...
    receipt = {
        **receipt_core,
        ...
        "extensions": {
            ...
            **(({"editor_provenance": editor_provenance}) if editor_provenance else {}),
            **(
...
def _redacted_email(email: str) -> str:
    """Return a deterministic, non-identifying email-shaped pseudonym."""
    normalized = _strip_terminal_escapes(email).strip().lower()
    if not normalized:
        return "redacted@users.noreply.aiir"
    digest = _sha256(normalized)[:16]
