...
# Also neutralize env-level diff overrides that bypass -c flags.
_GIT_SAFE_ENV: Dict[str, str] = {
    **os.environ,
    "GIT_TERMINAL_PROMPT": "0",
    "GIT_ASKPASS": "",
    "GIT_EXTERNAL_DIFF": "",  # Neutralize custom diff driver (belt-and-suspenders)
    "GIT_DIFF_OPTS": "",  # Neutralize env-level diff options
    "GIT_PAGER": "cat",  # Never paginate in a subprocess
}

# Ordered list of -c overrides that pin every git diff output-affecting knob
# to canonical defaults, making diff_hash byte-identical across user configs.
# Order matters: later entries win if git evaluates them left-to-right.
_GIT_DETERMINISTIC_CONFIG: list = [
    "-c",
    "core.quotepath=false",  # canonical paths (affects non-ASCII filenames)
    "-c",
    "diff.noprefix=false",  # pin a/ b/ prefixes
    "-c",
    "diff.mnemonicPrefix=false",  # prevent c/ i/ w/ o/ prefixes
    "-c",
    "diff.relative=false",  # full paths regardless of subdir/config
    "-c",
    "diff.context=3",  # pin hunk context (= -U3 default)
    "-c",
    "diff.algorithm=myers",  # pin algorithm (user may set patience/histogram)
    "-c",
    "diff.indentHeuristic=true",  # pin hunk-boundary heuristic (matches git default)
    "-c",
    "diff.renames=true",  # match git compiled default; user may set false/copies
    "-c",
    "diff.renameLimit=0",  # unlimited; max deployed commit=68 files << default 1000
    "-c",
    ...
    "-c",
    "diff.external=",  # neutralize GIT_EXTERNAL_DIFF/diff.external
    "-c",
    "color.ui=never",
    "-c",
    "color.diff=never",
    "-c",
    "core.pager=cat",  # never paginate in a subprocess (defensive)
    "-c",
    "log.showSignature=false",  # defensive; does not affect --format
    "-c",
    "i18n.logOutputEncoding=UTF-8",  # pin log output encoding (message_hash/subject stability)
    "-c",
    "i18n.commitEncoding=UTF-8",  # pin commit encoding (subject/body reads)
]

# Explicit diff flags that reinforce the -c config overrides above.
_GIT_DIFF_DETERMINISTIC_FLAGS: list = [
    "--no-color",
    "--no-ext-diff",
    "--no-textconv",
    "--src-prefix=a/",
...
# Unicode colon homoglyphs that NFKC does NOT normalize to ASCII ':'.
# These appear in "Co-authored-by<homoglyph>:" bypass attempts.
# Adding them here lets _normalize_for_detection catch trailer-key spoofing
# such as 'Co-authored-by꞉ Copilot' or 'Generated-by： Devin'.
_COLON_TO_ASCII = {
    "꞉": ":",  # MODIFIER LETTER COLON (꞉)
    "：": ":",  # FULLWIDTH COLON (：) — NFKC should cover, but be explicit
    "∶": ":",  # RATIO (∶)
    "˸": ":",  # MODIFIER LETTER RAISED COLON (˸)
    "⁚": ":",  # TWO DOT PUNCTUATION (⁚)
    "ः": ":",  # DEVANAGARI SIGN VISARGA (ः) — visually colon-like
    "ઃ": ":",  # GUJARATI SIGN VISARGA (ઃ)
}
...
def _run_git(
    args: List[str], cwd: Optional[str] = None, deterministic: bool = False
) -> str:
    """Run a git command and return stdout.

    When *deterministic* is True, prepends ``_GIT_DETERMINISTIC_CONFIG``
    after ``--no-optional-locks`` so that every output-affecting git knob is
    pinned to a canonical value regardless of the user's git config.  Use this
    for any invocation whose output feeds a content hash.
    """
    logger.debug("git %s (cwd=%s)", " ".join(args[:3]), cwd or os.getcwd())
    config_flags = _GIT_DETERMINISTIC_CONFIG if deterministic else []
    result = subprocess.run(
        ["git", "--no-optional-locks"] + config_flags + args,
...
def _hmac16(salt: Optional[str], msg: str) -> str:
    """Return HMAC-SHA256(salt, msg)[:16] as a lowercase hex string.

    Used to derive pseudonymous email tokens that are per-ledger-salt
    unlinkable.  When *salt* is ``None`` or empty, falls back to a fixed
    package-level constant so callers that have no ledger context still get
    a deterministic (though globally consistent) pseudonym rather than
    crashing.

    Args:
        salt: A 256-bit hex string (64 hex chars) from ``.aiir/config.json``,
              or ``None`` / empty string when no ledger is available.
        msg:  The message to authenticate (e.g. a normalized email address).

    Returns:
        A 16-character lowercase hex string.
    """
    _FALLBACK_SALT = b"aiir.redaction.v2"
    if salt:
        try:
...
def _hash_diff_streaming(parent: str, sha: str, cwd: Optional[str] = None) -> str:
    ...
    """
    # _GIT_DETERMINISTIC_CONFIG pins every output-affecting git knob so
    # diff_hash is byte-identical regardless of the user's git config.
    # _GIT_DIFF_DETERMINISTIC_FLAGS reinforce those settings at the argv level.
    # --no-ext-diff and --no-textconv also appear in the flags list to prevent
    # malicious .gitattributes from invoking custom diff drivers/textconv filters.
    proc = subprocess.Popen(
        [
            "git",
            "--no-optional-locks",
            *_GIT_DETERMINISTIC_CONFIG,
            "diff",
            *_GIT_DIFF_DETERMINISTIC_FLAGS,
            parent,
            sha,
        ],
        cwd=cwd or os.getcwd(),
...
def _normalize_for_detection(text: str) -> str:
    ...
    # Normalize Unicode colon homoglyphs to ASCII ':'
    # Must run before confusable map so we catch trailer-key bypass attempts
    # like 'Co-authored-by꞉ Copilot'.
    text = "".join(_COLON_TO_ASCII.get(c, c) for c in text)
