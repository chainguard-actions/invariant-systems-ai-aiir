...
class _NoRedirectHandler(HTTPRedirectHandler):
    """Refuse to auto-follow API redirects.

    Automatically following a redirect would carry the Authorization header to
    the redirect target, which may be a different origin (cross-origin token
    leakage).  We raise instead so a redirect is surfaced rather than silently
    followed with credentials attached.
    """

    def redirect_request(self, req, fp, code, msg, headers, newurl):
        raise RuntimeError(
            f"GitHub API request was redirected ({code}) to {newurl!r}. "
            "AIIR refuses to follow API redirects to prevent token leakage."
        )
...
def urlopen(req, timeout=None):  # noqa: A001 - module-level no-redirect seam (patchable in tests)
    """Open ``req`` without following redirects (see :class:`_NoRedirectHandler`).

    Defined at module scope so tests can patch ``aiir._github.urlopen`` and so
    every API call routes through the same credential-safe, no-redirect opener.
    """
...
def _github_api_request(
    ...
    # Validate URL scheme: only HTTPS is permitted for GitHub API calls.
    # HTTP targets are rejected outright — auth tokens must not travel in clear.
    from urllib.parse import urlparse as _urlparse

    _parsed = _urlparse(endpoint)
    if _parsed.scheme != "https":
        raise RuntimeError(
            f"Refusing API request to non-HTTPS URL (scheme={_parsed.scheme!r}). "
            "GitHub API calls require HTTPS to protect the Authorization token."
        )
...
def create_check_run(
    ...
    # C5.7: Verify each receipt and set conclusion based on results.
    # Empty receipts => success (nothing to fail).
    # Any invalid receipt => failure, with FAILED marker in title/summary.
    if receipts:
        all_valid = all(verify_receipt(r).get("valid") for r in receipts)
    else:
        all_valid = True

    conclusion = "success" if all_valid else "failure"
    ...
    if not all_valid:
        summary_lines.insert(
            0, "**FAILED** — one or more receipts failed verification.\n"
        )
...
def post_pr_comment(
    ...
    # Validate repo shape to prevent path/query injection.
...
def _validate_repo_shape(repo: str) -> None:
    """Reject repo values that don't match the ``owner/repo`` pattern.

    Path-traversal segments (``../``), query strings, and any character
    outside the GitHub-allowed set would let a caller alter the API request
    path or inject query parameters.  We allowlist the safe pattern and reject
    everything else.

    Raises:
        RuntimeError: If ``repo`` does not match ``owner/repo``.
    """
    if not _REPO_SHAPE_RE.match(repo):
        raise RuntimeError(
            f"Invalid repo shape (must be 'owner/repo', got {repo[:60]!r}). "
            "Path traversal and query injection are not permitted."
        )
...
def _find_existing_comment(
    ...
    Routes the GET through the same HTTPS-scheme guard as every other API
    call: if ``GITHUB_API_URL`` is not an HTTPS URL the function raises
    immediately rather than sending the Authorization token in cleartext.
    """
    auth_token = token or os.environ.get("GITHUB_TOKEN", "")
    if not auth_token:
        return None

    # Validate repo shape before building the URL to prevent path/query injection.
    _validate_repo_shape(repo)

    api_url = os.environ.get("GITHUB_API_URL", "https://api.github.com")

    # Enforce HTTPS *before* attaching credentials — same guard as
    # _github_api_request (~207-212).  An http:// GITHUB_API_URL would send
    # the Bearer token in cleartext; we raise here instead.
    from urllib.parse import urlparse as _urlparse
    ...
    try:
        ...
        raise  # Propagate HTTPS/scheme errors — not swallowable
    except Exception:
