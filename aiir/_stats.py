...
from aiir._core import _USE_BOXDRAW, _strip_terminal_escapes
...
def format_stats(
    ...
    ai_pct = index.get("ai_percentage", 0.0)
    signed_count = index.get("signed_receipt_count", 0)
    unsigned_count = index.get("unsigned_receipt_count", 0)
    ...
    if config and config.get("namespace"):
        safe_ns = _strip_terminal_escapes(str(config["namespace"]))[:200]
        ns = f"  Namespace: {safe_ns}\n"
    iid = ""
    ...
    return (
        ...
        f" ({ai_pct}%)\n"
        f"  Signing:      {signed_count} signed, {unsigned_count} unsigned\n"
        f"  First:        {first}\n"
        f"  Latest:       {latest}\n"
        f"{ns}"
        f"{iid}"
