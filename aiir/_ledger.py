...
def _receipt_is_signed(receipt: Dict[str, Any]) -> bool:
    """Return True when a receipt carries an embedded Sigstore bundle reference."""
    extensions = receipt.get("extensions", {})
    if not isinstance(extensions, dict):
        return False
    return bool(extensions.get("sigstore") or extensions.get("sigstore_bundle"))
...
def append_to_ledger(
    ...
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        for receipt in receipts:
            ...
            if is_signed:
                index["signed_receipt_count"] = index.get("signed_receipt_count", 0) + 1
            else:
