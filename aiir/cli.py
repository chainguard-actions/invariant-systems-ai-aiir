...
def _verification_target_label(result: Dict[str, Any]) -> str:
    ...
    if result.get("receipt_type") == "agent_receipt":
        tool = _strip_terminal_escapes(str(result.get("actor_tool", "unknown")))[:40]
        action = _strip_terminal_escapes(str(result.get("action_kind", "")))[:40]
        return f"agent={tool} ({action})" if action else f"agent={tool}"
...
def _expand_shorthand_command(argv: Sequence[str]) -> List[str]:
    # BREADCRUMB (cli-restructure): this is the *partial* discoverable-verb
    # surface — single-word verbs rewritten to the canonical flat flags. The
    # full subcommand restructure (grouped subparsers, per-verb --help, the
    # remaining mode verbs like verify-release/export/review/commitment/
    # policy-init, and handler extraction) is intentionally deferred. The
    # `aiir agent` block in main() is the reference pattern for a real nested
    # subcommand. Plan + remaining work: docs/reference/cli-subcommands.md (Roadmap).
    if not argv:
        return list(argv)
    ...
    mapping = {
        ...
        # Discoverable verbs that map onto the flat-flag engine. The flat flags
        # remain the canonical, backward-compatible form; these are sugar.
        "verify": ["--verify"],
        "stats": ["--stats"],
        "badge": ["--badge"],
        "doctor": ["--doctor"],
...
def _parse_agent_artifact(spec: str) -> Dict[str, str]:
    """Parse a ``TYPE:REF`` agent artifact spec (REF may contain colons)."""

    text = _strip_terminal_escapes(str(spec)).strip()
    art_type, sep, ref = text.partition(":")
    art_type = art_type.strip()
    ref = ref.strip()
    if not sep or not art_type or not ref:
        raise ValueError(
            f"Invalid artifact {spec!r}. Use TYPE:REF, e.g. file:src/app.py."
        )
    return {"type": art_type, "ref": ref}


def _build_agent_parser() -> argparse.ArgumentParser:
    """Build the ``aiir agent`` subcommand parser."""

    parser = argparse.ArgumentParser(
        prog="aiir agent",
        description="Emit and verify AIIR agent receipts (aiir/agent_receipt.v0.1).",
    )
    sub = parser.add_subparsers(dest="agent_cmd")

    emit = sub.add_parser("emit", help="Emit an agent receipt for one agent action")
    emit.add_argument(
        "--action", required=True, help="action kind (edit, run, test, …)"
    )
    emit.add_argument(
        "--tool", required=True, help="agent tool name (e.g. claude-code)"
    )
    emit.add_argument(
        "--kind", default="agent", help="actor kind: agent, bot, human, mixed"
    )
    emit.add_argument("--surface", default=None, help="tool surface (e.g. cli, ide)")
    emit.add_argument("--session", default=None, help="opaque session reference")
    emit.add_argument("--intent", default=None, help="action intent")
    emit.add_argument("--summary", default=None, help="one-line action summary")
    emit.add_argument(
        "--input",
        action="append",
        default=None,
    ...
    emit.add_argument(
        ...
        help="policy decision: allowed, warned, blocked, needs_review, not_evaluated",
    )
    emit.add_argument(
        "--policy-reason",
        action="append",
        default=None,
        dest="policy_reasons",
        help="policy reason (repeatable)",
    )
    emit.add_argument(
        "--policy-contract", default=None, help="policy contract reference"
    )
    emit.add_argument(
        "--ledger", default=None, help=f"ledger dir (default {_LEDGER_DIR})"
    )
    emit.add_argument(
        "--json",
        action="store_true",
        help="print the receipt to stdout instead of writing the ledger",
    )

    verify = sub.add_parser("verify", help="Verify an agent receipt file")
    verify.add_argument("file", help="path to an agent receipt JSON file")
    return parser


def _run_agent_subcommand(argv: Sequence[str]) -> int:
    """Handle ``aiir agent emit|verify`` (a self-contained subcommand).

    Kept separate from the flat-flag engine so the agent-receipt surface stays
    clean while existing flat invocations are completely unaffected.
    """

    parser = _build_agent_parser()
    args = parser.parse_args(list(argv))

    if args.agent_cmd == "emit":
        try:
            inputs = [_parse_agent_artifact(s) for s in (args.input or [])]
            outputs = [_parse_agent_artifact(s) for s in (args.output or [])]
            receipt = build_agent_receipt(
                actor_kind=args.kind,
                actor_tool_name=args.tool,
                action_kind=args.action,
                actor_tool_surface=args.surface,
                actor_session_ref=args.session,
                action_intent=args.intent,
                action_summary=args.summary,
                artifact_inputs=inputs or None,
                artifact_outputs=outputs or None,
                policy_decision=args.policy,
                policy_reasons=args.policy_reasons,
                policy_contract_ref=args.policy_contract,
            )
        except ValueError as exc:
            print(f"{_e('error')} {exc}", file=sys.stderr)
            return 1
        ...
        print(format_agent_receipt_pretty(receipt), file=sys.stderr)
        print(
            f"   {_e('hint')} Saved to {ledger_dir}/receipts.jsonl",
            file=sys.stderr,
        )
        return 0

    if args.agent_cmd == "verify":
        path = Path(args.file)
        # Apply the same read-side guards as verify_receipt_file: refuse
        # symlinks (path-probing) and oversized files (memory exhaustion).
        if path.is_symlink():
            print(
                f"{_e('error')} refusing to verify a symlink: {args.file}",
                file=sys.stderr,
            )
            return 1
        try:
            if path.stat().st_size > MAX_RECEIPT_FILE_SIZE:
                print(
                    f"{_e('error')} file too large (max {MAX_RECEIPT_FILE_SIZE} bytes): {args.file}",
                    file=sys.stderr,
                )
                return 1
            data = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, ValueError) as exc:
            print(f"{_e('error')} cannot read {args.file}: {exc}", file=sys.stderr)
            return 1
        if not is_agent_receipt(data):
            print(
                f"{_e('error')} not an agent receipt: {args.file}",
                file=sys.stderr,
            )
            return 1
        result = verify_agent_receipt(data)
        if result.get("valid"):
            print(
                f"{_e('ok')} agent receipt valid: {result.get('record_id')}",
                file=sys.stderr,
            )
            return 0
        print(
            f"{_e('error')} agent receipt INVALID: {result.get('errors') or 'hash mismatch'}",
            file=sys.stderr,
        )
        return 1
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # The `agent` subcommand is self-contained (its own clean emit/verify flags)
    # and does not flow through the flat-flag parser below.
    # BREADCRUMB (cli-restructure): this is the reference pattern for the full
    # subcommand restructure — add more nested subcommands here the same way,
    # then thin out _expand_shorthand_command. See docs/reference/cli-subcommands.md.
    if cli_argv and cli_argv[0].strip().lower() == "agent":
        return _run_agent_subcommand(cli_argv[1:])
    raw_argv = _expand_shorthand_command(cli_argv)
    parser = _FriendlyParser(
        ...
        epilog=(
            ...
            "  aiir --redact-emails        Pseudonymize emails (per-ledger HMAC)\n"
            "\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(https://in-toto.io/Statement/v1) for the supply-chain "
            "attestation ecosystem (SLSA, Sigstore "
    ...
    parser.add_argument(
        ...
        dest="release_bundle",
        help=(
            "Build a self-contained release evidence bundle directory: receipts, "
            "policy, Verification Summary Attestation, evidence summary, auditor "
            "report, and verifier instructions. Use with --range, --receipts, "
            "--policy. Combine with --emit-report (default on) and --overwrite."
        ),
    )
    parser.add_argument(
        "--no-emit-report",
        action="store_true",
        dest="no_emit_report",
        help="With --release-bundle: skip the human-readable auditor-report.html",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        dest="overwrite",
        help="With --release-bundle: write into a non-empty output directory",
    )
    parser.add_argument(
        "--redact-files",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Replace author and committer emails with per-ledger HMAC-SHA256 pseudonyms "
            "(format: redacted+<16hex>@users.noreply.aiir). The salt is stored in "
            ".aiir/config.json (0600) and is required to reproduce the same pseudonym. "
            "Cross-ledger pseudonyms intentionally differ for privacy."
        ),
    )
    parser.add_argument(
        "--namespace",
    ...
    if args.quickstart:
        ...
        try:
            ...
            err_str = str(e)
            # Detect empty repo (no commits yet) and emit a friendly message.
            if "unknown revision" in err_str or "bad default revision" in err_str:
                print(
                    f"{_e('error')} This repository has no commits yet.",
                    file=sys.stderr,
                )
                print(
                    f"   {_e('hint')} Make your first commit, then run `aiir --quickstart` again.",
                    file=sys.stderr,
                )
            else:
                print(f"{_e('error')} {e}", file=sys.stderr)
    ...
    if args.status:
        ...
        # repo_label reflects both git availability AND being inside a repo.
        _status_head_sha = state.get("head_sha")
        if not state.get("git_available"):
            repo_label = "unavailable"
        elif _status_head_sha:
            repo_label = "clean"
        else:
            repo_label = "no-repo"
        ledger_dir = args.ledger if args.ledger else _LEDGER_DIR
        ...
        # last_commit_status is "verified" only when we have a real HEAD SHA
        # and that receipt is present in the ledger.
        if not _status_head_sha:
            last_commit_status = "unavailable"
        elif state.get("head_receipt_status") == "present":
            last_commit_status = "verified"
        else:
            last_commit_status = "missing"
        ...
        if args.json_stdout:
            ...
            print(
                f"AI usage: {'detected' if ai_receipts_count > 0 else 'none'}"
                f" ({ai_receipts_count} commits)",
                file=sys.stderr,
            )
            print(
    ...
    if args.ci:
        ...
        # Empty ledger passes verification structurally but is vacuously
        # successful — detecting 0 receipts is a misconfiguration, not success.
        if total == 0:
            message = (
                "Ledger is empty (0 receipts). Run 'aiir' first to generate receipts."
            )
            if args.json_stdout:
                print(
                    json.dumps(
    ...
    if args.init:
        ...
        if args.policy:
            if args.policy not in POLICY_PRESETS:
                valid_presets = ", ".join(sorted(POLICY_PRESETS))
                print(
                    f"{_e('error')} Unknown policy preset {args.policy!r}."
    ...
    if args.review is not None:
        ...
        # Validate and resolve the ref to a full SHA — the review receipt
        # schema requires a hex SHA (^[0-9a-f]{40}$ or ^[0-9a-f]{64}$), not a
        # symbolic ref like "HEAD" or a short SHA like "abc1234".
        # Validate first to prevent option injection into git rev-parse.
        reviewed_ref = args.review
        try:
            _validate_ref(reviewed_ref)
        except ValueError as e:
            print(f"{_e('error')} {e}", file=sys.stderr)
            return 1
        try:
        ...
        if args.json_stdout:
            ...
            # Default mode: persist to ledger, verify, and report the path.
            ledger_dir = args.ledger if args.ledger is not None else _LEDGER_DIR
            try:
                _rv_appended, _rv_skipped, _rv_ledger_path = append_to_ledger(
                    [review_receipt],
                    ledger_dir=ledger_dir,
                )
            except ValueError as e:
                print(f"{_e('error')} {e}", file=sys.stderr)
                return 1
            # Verify the receipt we just built (integrity self-check).
            _rv_verify = verify_receipt(review_receipt)
            if not _rv_verify.get("valid"):
                _rv_errs = _rv_verify.get("errors", [])
                _rv_err_msg = _rv_errs[0] if _rv_errs else "integrity check failed"
                print(
                    f"{_e('error')} Review receipt failed verification: {_rv_err_msg}",
                    file=sys.stderr,
                )
                return 1
        ...
        if not args.quiet:
            ...
            if not (args.json_stdout or args.jsonl):
                print(
                    f"   Ledger: {_rv_ledger_path}",
                    file=sys.stderr,
                )
    ...
    if args.artifact:
        ...
        if result.get("valid"):
            print(
                f"{_e('ok')} Artifact transparency verified.",
                file=sys.stderr,
            )
    ...
    if args.verify:
        ...
        # Catch directory targets early — verify_receipt_file raises
        # IsADirectoryError on them, which would produce a raw traceback.
        if verify_target.is_dir():
            print(
                f"{_e('error')} {args.verify!r} is a directory, not a receipt file.",
                file=sys.stderr,
            )
            print(
                f"   {_e('hint')} Point --verify at a .json receipt file or a .jsonl ledger.",
                file=sys.stderr,
            )
            return 1
        ...
        if result.get("valid"):
            if verify_is_ledger:
                ...
                # Empty ledger passes structurally but is vacuously successful.
                if count == 0:
                    print(
                        f"{_e('error')} Ledger is empty (0 receipts)."
                        " Run 'aiir' first to generate receipts.",
                        file=sys.stderr,
                    )
                    return 1
                print(
                    f"{_e('ok')} All good! {valid_receipts}/{count} ledger receipts verified.",
                    file=sys.stderr,
                )
                if args.verify_signature:
                    # --verify-signature cannot be applied to JSONL ledgers
                    # (no single signable artifact); error clearly instead of
                    # silently passing.
                    print(
                        f"{_e('error')} --verify-signature cannot be applied to"
                        " a .jsonl ledger (no single signable artifact)."
                        " Verify individual receipt .json files instead.",
                        file=sys.stderr,
                    )
                    return 1
            ...
            if rtype == "inference_receipt" or (
                ...
                if "total" in result:
                    ...
                    print(
                        ...
                        f"granularity={g}, tokens={tc}.",
                        file=sys.stderr,
                    )
                if args.verify_signature:
                    # --verify-signature is not applicable to inference receipts
                    # (they are not signed with Sigstore); error clearly.
                    print(
                        f"{_e('error')} --verify-signature is not applicable to"
                        " inference receipts (Sigstore signing is not supported"
                        " for this receipt type).",
                        file=sys.stderr,
                    )
                    return 1
                print(json.dumps(result, indent=2))
            ...
            if rtype == "commitment_receipt" and "receipts" not in result:
                print(
                    f"{_e('ok')} Commitment receipt verified.",
                    file=sys.stderr,
                )
                print(
            ...
            if rtype == "research_evidence_receipt":
                if "total" in result:
                    ...
                    print(
                        ...
                        f"disclosure_tier={result.get('disclosure_tier', 'unknown')}.",
                        file=sys.stderr,
                    )
                if args.verify_signature:
                    # --verify-signature is not applicable to research evidence
                    # receipts (Sigstore signing is not supported for this
                    # receipt type); error clearly instead of silently passing.
                    print(
                        f"{_e('error')} --verify-signature is not applicable to"
                        " research evidence receipts (Sigstore signing is not"
                        " supported for this receipt type).",
                        file=sys.stderr,
                    )
                    return 1
                print(json.dumps(result, indent=2))
            ...
            print(
                f"{_e('ok')} All good! {count} receipt{'s' if count != 1 else ''} verified.",
                file=sys.stderr,
            )
            if "receipts" in result:
                for r in result["receipts"]:
                    # Agent receipts identify by record_id; commit/review by receipt_id.
                    _rid = (r.get("receipt_id") or r.get("record_id") or "")[:24]
                    print(
                        f"   {_rid}... {_verification_target_label(r)} {_e('check')}",
                        file=sys.stderr,
                    )
            else:
                _rid = (result.get("receipt_id") or result.get("record_id") or "")[:24]
                print(
                    f"   {_rid}... {_verification_target_label(result)} {_e('check')}",
                    file=sys.stderr,
                )
    ...
    # --- Release-bundle mode ---
    if getattr(args, "release_bundle", None):
        rb_receipts_path = args.receipts or ".aiir/receipts.jsonl"
        rb_policy_preset = None
        rb_policy_path = None
        if args.policy:
            from aiir._policy import POLICY_PRESETS as _PP

            if args.policy in _PP:
                rb_policy_preset = args.policy
            else:
                rb_policy_path = args.policy

        try:
            bundle = build_release_bundle(
                commit_range=args.range_spec,
                receipts_path=rb_receipts_path,
                policy_path=rb_policy_path,
                policy_preset=rb_policy_preset,
                subject_name=getattr(args, "subject", None),
                emit_report=not getattr(args, "no_emit_report", False),
        ...
        print(format_bundle_summary(bundle, out_dir), file=sys.stderr)
        print(json.dumps(bundle["manifest"], indent=2))

        result_code = int(bundle.get("result_code", 1))
        if result_code == 0:
            print(f"\n{_e('ok')} Release bundle written: {out_dir}", file=sys.stderr)
        else:
            print(
                f"\n{_e('error')} Release bundle written (verification FAILED): "
                f"{bundle.get('reason', '')}",
                file=sys.stderr,
            )
        return result_code
    ...
    if use_ledger or namespace or args.redact_emails:
        ...
        # Resolve per-ledger redaction salt only when --redact-emails is set.
        # Do NOT create the salt when --redact-emails is absent (C6).
        if args.redact_emails:
            try:
                redaction_salt = _ensure_redaction_salt(
                    config, _config_path(ledger_cfg_dir)
                )
            except OSError:  # pragma: no cover — filesystem error
                redaction_salt = config.get("redaction_salt")  # use in-memory only
    ...
    if getattr(args, "gl_sast_report", None):
        ...
        _cwd_resolved = _Path.cwd().resolve()
        # Resolve the target the same way as the cwd so the containment check is
        # consistent across platform path quirks: macOS routes temp dirs through
        # the /var -> /private/var symlink, and Windows normalizes drive-letter
        # case and 8.3 short names.  Comparing a raw path against a resolved cwd
        # produced false rejections of legitimate in-tree paths on both.
        _sast_resolved = _sast_raw.resolve()

        # Reject symlinks on the target path itself (O_NOFOLLOW-style guard).
        # Checked before containment so the symlink-specific message is emitted
        # even when a platform temp symlink would otherwise trip containment.
        if _sast_raw.is_symlink() or _sast_resolved.is_symlink():
            print(
                f"{_e('error')} SAST report path must not be a symbolic link.",
                file=sys.stderr,
            )
            return 1

        # The fully resolved target must live inside the resolved working dir.
        # Rejects absolute paths outside the project and "../" traversal escapes.
        try:
            _sast_resolved.relative_to(_cwd_resolved)
        except ValueError:
            print(
                f"{_e('error')} SAST report path must be within the working directory"
                " (paths outside the project are not allowed).",
                file=sys.stderr,
            )
            return 1

        sast_report = format_gl_sast_report(receipts)
        sast_path = _sast_resolved
        sast_path.parent.mkdir(parents=True, exist_ok=True)
        # Re-check after mkdir (narrow TOCTOU/symlink-attack window).
        if sast_path.is_symlink():
            print(
                f"{_e('error')} SAST report target became a symlink after directory creation.",
                file=sys.stderr,
            )
            return 1
