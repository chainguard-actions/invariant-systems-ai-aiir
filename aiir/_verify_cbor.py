...
from aiir._commitment import COMMITMENT_CORE_KEYS, COMMITMENT_RECEIPT_TYPE
...
def decode_cbor(data: bytes, offset: int = 0, _depth: int = 0) -> Tuple[Any, int]:
    ...
    - Map keys sorted bytewise (lexicographically) over their encoded CBOR bytes
    ...
    *_depth* is an internal recursion counter.  Raises CborDecodeError when
    ...
    if major == _MT_MAP:
        ...
        for _ in range(count):
            ...
            # RFC 8949 section 4.2.1: bytewise lexicographic over encoded key bytes
            ...
            # Distinct CBOR encodings can map to keys that are equal as Python
            # objects (e.g. integer 0 and float 0.0, or 1 and True): they pass
            # the bytewise-order check above but would silently collapse in the
            # result dict, losing a key.  Reject such maps rather than decode
            # lossily (defends direct decode_cbor_full callers, not just the
            # sidecar round-trip).
            if key in result:
                raise CborDecodeError("duplicate CBOR map key after decoding")
            result[key] = value
        return result, off
    ...
    *,
    ...
    if json_receipt is not None and isinstance(json_receipt, dict):
        CORE_KEYS = (
            ...
            if json_receipt.get("type") == COMMITMENT_RECEIPT_TYPE
            else {
                "type",
                "schema",
                "version",
