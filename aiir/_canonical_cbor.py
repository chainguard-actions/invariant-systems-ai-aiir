...
Map keys are sorted bytewise (lexicographically) over their encoded CBOR bytes
...
def _encode_uint(major: int, n: int) -> bytes:
    ...
    if n >= 0x1_0000_0000_0000_0000:  # 2**64
        raise ValueError("integer too large for canonical CBOR (>= 2**64)")
    if n < 24:
...
def _encode_float_canonical(x: float) -> bytes:
    ...
    try:
        ...
        pass  # Large finite float not representable as f32 — fall through to f64
    return b"\xfb" + want
...
def _try_f16(x: float) -> bytes | None:
    try:
        ...
        return None  # Not f32-representable, so not f16-representable either
...
def _encode_item(obj: Any, _depth: int = 0) -> bytes:
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            encoded_items.append((key_bytes, value))
