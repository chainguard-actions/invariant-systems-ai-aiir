...
def consume_editor_provenance(
    ...
    try:
        ...
        # NOTE: this 'provable' tier mode has auto-repair for orphaned chain heads
        # (a record whose previousRecordHash doesn't connect to the ledger start).
        # Auto-repair means a broken chain is silently re-linked rather than raised,
        # which is intentional for the IDE-context tier but is NOT cryptographic proof
        # of authorship — it is a best-effort record for audit trail purposes.
        # The 'provable' label here means the chain is internally consistent, not that
        # it binds to a specific commit via a cryptographic commitment.
        # TODO (WS-F): Full signing/commit binding is out of scope for this tier.
        repaired = None
        if (
            str(exc)
            == "editor provenance queue has a broken hash chain at the first record"
        ...
        if repaired is None:
            # Surface the breakage — do not silently swallow chain validation errors
            # other than the known orphaned-head case.
            raise
        records = repaired
        path.write_text(
            "\n".join(
