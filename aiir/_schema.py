...
_RE_SHA_HEX = re.compile(r"^(?:[0-9a-f]{40}|[0-9a-f]{64})\Z")
_RE_HASH_PREFIX = re.compile(r"^sha256:[0-9a-f]{64}\Z")
_RE_VERSION = re.compile(r"^[0-9]+\.[0-9]+\.[0-9]+([.+\-][0-9a-zA-Z.+\-]*)?\Z")
_RE_RECEIPT_ID = re.compile(r"^g1-[0-9a-f]{32}\Z")
_RE_CONTENT_HASH = re.compile(r"^sha256:[0-9a-f]{64}\Z")
_RE_TOOL_URI = re.compile(r"^https://[^\s/\x00-\x1f\x7f][^\s\x00-\x1f\x7f]*\Z")
...
def validate_receipt_schema(receipt: Any) -> List[str]:
    ...
    _validate_commit(commit, errors, receipt.get("schema"))
...
def _validate_commit(
    commit: Dict[str, Any], errors: List[str], schema: Optional[str] = None
) -> None:
    ...
    # v2 DAG binding fields — presence is required for aiir/commit_receipt.v2
    if schema == "aiir/commit_receipt.v2":
        if "tree_sha" not in commit:
            errors.append("commit.tree_sha is required for aiir/commit_receipt.v2")
        if "parent_shas" not in commit:
            errors.append("commit.parent_shas is required for aiir/commit_receipt.v2")
...
def _validate_provenance(prov: Dict[str, Any], errors: List[str]) -> None:
    ...
    if isinstance(tool, str) and not _RE_TOOL_URI.match(tool):
        errors.append(
            f"{prefix}.tool must be an https URI "
            f"(e.g. 'https://github.com/invariant-systems-ai/aiir@1.6.0'), got {tool!r}"
        )
