...
# NOTE: We advertise 2024-11-05 (not 2025-03-26) because 2025-03-26 mandates
# JSON-RPC batch support which this server does not implement.  Using the older
# version keeps the advertised capability set honest.  Batch requests receive a
# -32600 Invalid Request response rather than silently being dropped.
...
TOOLS = [
    {
        ...
        "description": (
            ...
            "NOTE: this MCP tool returns receipt data but does NOT write to the ledger "
            "— use the aiir CLI to persist receipts. The tool is read-only/ephemeral by design. "
...
def serve_stdio() -> None:
    ...
    # IMPORTANT: we enforce this DURING read (bounded readline) so that a
    # newline-less message cannot buffer arbitrarily large data before the
    # size check.  Reading via `for line in sys.stdin` buffers the entire
    # line before we can check — a single giant line-less payload would OOM.
    _MAX_MSG_SIZE = 10 * 1024 * 1024  # 10 MB
    _EOF_SENTINEL = object()  # distinct object to signal EOF vs oversized discard

    def _read_bounded_line():
        """Read one line from stdin, bounded by _MAX_MSG_SIZE bytes.

        Returns:
          - The stripped line string when a line is successfully read.
          - _EOF_SENTINEL when stdin is at EOF.
          - None when the line was oversized and discarded.

        Raises nothing — invalid UTF-8 bytes are replaced with U+FFFD so a
        single bad byte cannot crash the server (defensive decode).
        """
        chunks: list = []
        total = 0
        raw_in = getattr(sys.stdin, "buffer", None)
        if raw_in is None:
            # Fallback: text-mode stdin (e.g. pytest capture) — read char by char.
            while True:
                ch = sys.stdin.read(1)
                if not ch:
                    # EOF — only return sentinel if nothing was read yet
                    if not chunks:
                ...
                if total > _MAX_MSG_SIZE:
                    # Drain remaining bytes on this line (discard).
                    while True:
                        ch2 = sys.stdin.read(1)
                        if not ch2 or ch2 == "\n":
                            break
                    return None  # Oversized line — discard
                chunks.append(ch)
            return "".join(chunks).strip()
        else:
            # Binary-mode stdin — read byte by byte until \n or limit.
            while True:
                byte = raw_in.read(1)
                if not byte:
                    # EOF
            ...
            # Defensive decode: replace invalid UTF-8 bytes instead of crashing.
    ...
    while True:
        ...
        if line is _EOF_SENTINEL:
            break  # Clean EOF — exit the server loop
        if line is None:
            continue  # Oversized line was discarded — keep looping
        # Empty string after stripping (blank line) — skip
        if not line:
            continue
        ...
        # JSON-RPC batch arrays: this server does not implement batch support.
        # Protocol version 2024-11-05 (which we advertise) does not mandate
        # batch handling, so we return a proper -32600 Invalid Request error.
        if isinstance(msg, list):
            _send(_make_error(None, -32600, "Batch requests are not supported"))
            continue

        # Reject non-object JSON (e.g., strings, ints, null).
        ...
        # Sliding-window rate limiter — applied to ALL inbound messages
        # (including notifications) to prevent local DoS via subprocess flooding.
        # Previously notifications bypassed this check, allowing unbounded
        # side-effecting handler calls without throttling.
        now = time.monotonic()
        ...
        if len(_request_times) > _RATE_LIMIT_MAX:
            ...
            # For notifications (no id), silently drop — per JSON-RPC 2.0 §4.1
