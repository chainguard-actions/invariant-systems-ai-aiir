...
class _NoRedirectHandler(HTTPRedirectHandler):
    """Refuse to auto-follow API redirects to prevent cross-origin token leakage.

    Automatically following a redirect would carry auth headers to the redirect
    target, so we raise instead of silently following with credentials attached.
    """

    def redirect_request(self, req, fp, code, msg, headers, newurl):
        raise RuntimeError(
            f"GitLab API request was redirected ({code}) to {newurl!r}. "
...
def urlopen(req, timeout=None):  # noqa: A001 - module-level no-redirect seam (patchable in tests)
    ...
    Defined at module scope so tests can patch ``aiir._gitlab.urlopen`` and so
...
def _gitlab_api_request(
    ...
    # Validate URL scheme: only HTTPS is permitted for GitLab API calls.
    ...
    if _parsed.scheme != "https":
        raise RuntimeError(
            ...
            "GitLab API calls require HTTPS to protect authentication tokens."
        )
...
def query_gitlab_graphql(
    ...
    # Reject non-HTTPS GraphQL targets to protect the Authorization token.
    from urllib.parse import urlparse as _urlparse

    _parsed = _urlparse(base)
    if _parsed.scheme != "https":
        raise RuntimeError(
            f"Refusing GraphQL request to non-HTTPS URL (scheme={_parsed.scheme!r}). "
            "GitLab API calls require HTTPS to protect authentication tokens."
        )
